...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object jobs {
		...
		object create {
			def apply(onBehalfOfContentOwner: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v1/jobs").addQueryStringParameters("onBehalfOfContentOwner" -> onBehalfOfContentOwner.toString))
		}
		class delete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Empty]) {
			def apply() = auth.exec(req,_.execute("DELETE")).map(_.json.as[Schema.Empty])
		}
		object delete {
			def apply(onBehalfOfContentOwner: String, jobId: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v1/jobs/${jobId}").addQueryStringParameters("onBehalfOfContentOwner" -> onBehalfOfContentOwner.toString))
		...
		object get {
			def apply(onBehalfOfContentOwner: String, jobId: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/jobs/${jobId}").addQueryStringParameters("onBehalfOfContentOwner" -> onBehalfOfContentOwner.toString))
		...
		object reports {
			...
			object get {
				def apply(onBehalfOfContentOwner: String, jobId: String, reportId: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/jobs/${jobId}/reports/${reportId}").addQueryStringParameters("onBehalfOfContentOwner" -> onBehalfOfContentOwner.toString))
	...
	object reportTypes {
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListReportTypesResponse]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListReportTypesResponse])
		}
		object list {
	...
	object media {
		class download(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GdataMedia]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.GdataMedia])
		}
		object download {
