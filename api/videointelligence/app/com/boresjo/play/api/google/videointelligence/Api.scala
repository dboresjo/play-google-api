...
class Api @Inject() (ws: WSClient) extends PlayApi {
		...
		object locations {
			object operations {
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleLongrunning_ListOperationsResponse]) {
					def apply() = req.execute("GET").map(_.json.as[Schema.GoogleLongrunning_ListOperationsResponse])
				}
				object list {
				...
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleLongrunning_Operation]) {
					def apply() = req.execute("GET").map(_.json.as[Schema.GoogleLongrunning_Operation])
				}
				object get {
				...
				class delete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleProtobuf_Empty]) {
					def apply() = req.execute("DELETE").map(_.json.as[Schema.GoogleProtobuf_Empty])
				}
				object delete {
				...
				class cancel(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withGoogleLongrunning_CancelOperationRequest(body: Schema.GoogleLongrunning_CancelOperationRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.GoogleProtobuf_Empty])
				}
				object cancel {
		...
		object projects {
			object locations {
				object operations {
					...
					object get {
						def apply(projectsId :PlayApi, locationsId :PlayApi, operationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v1/operations/projects/${projectsId}/locations/${locationsId}/operations/${operationsId}")).addQueryStringParameters("name" -> name.toString))
						given Conversion[get, Future[Schema.GoogleLongrunning_Operation]] = (fun: get) => fun.apply()
					}
					class delete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleProtobuf_Empty]) {
						def apply() = req.execute("DELETE").map(_.json.as[Schema.GoogleProtobuf_Empty])
					...
					object delete {
						def apply(projectsId :PlayApi, locationsId :PlayApi, operationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"v1/operations/projects/${projectsId}/locations/${locationsId}/operations/${operationsId}")).addQueryStringParameters("name" -> name.toString))
						given Conversion[delete, Future[Schema.GoogleProtobuf_Empty]] = (fun: delete) => fun.apply()
					}
					class cancel(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleProtobuf_Empty]) {
						def apply() = req.execute("POST").map(_.json.as[Schema.GoogleProtobuf_Empty])
					}
					object cancel {
						def apply(projectsId :PlayApi, locationsId :PlayApi, operationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): cancel = new cancel(auth(ws.url(BASE_URL + s"v1/operations/projects/${projectsId}/locations/${locationsId}/operations/${operationsId}:cancel")).addQueryStringParameters("name" -> name.toString))
	...
	object videos {
		class annotate(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withGoogleCloudVideointelligenceV1_AnnotateVideoRequest(body: Schema.GoogleCloudVideointelligenceV1_AnnotateVideoRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.GoogleLongrunning_Operation])
		}
		object annotate {
			def apply()(using auth: AuthToken, ec: ExecutionContext): annotate = new annotate(auth(ws.url(BASE_URL + s"v1/videos:annotate")).addQueryStringParameters())
		}
	}
}
