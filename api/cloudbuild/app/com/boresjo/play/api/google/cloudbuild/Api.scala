...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object projects {
		object locations {
			...
			object list {
				def apply(projectsId :PlayApi, pageToken: String, pageSize: Int, name: String, filter: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations")).addQueryStringParameters("pageToken" -> pageToken.toString, "pageSize" -> pageSize.toString, "name" -> name.toString, "filter" -> filter.toString))
			...
			object connections {
				...
				object getIamPolicy {
					def apply(projectsId :PlayApi, locationsId :PlayApi, connectionsId :PlayApi, resource: String)(using auth: AuthToken, ec: ExecutionContext): getIamPolicy = new getIamPolicy(auth(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/connections/${connectionsId}:getIamPolicy")).addQueryStringParameters("resource" -> resource.toString))
				...
				object setIamPolicy {
					def apply(projectsId :PlayApi, locationsId :PlayApi, connectionsId :PlayApi, resource: String)(using auth: AuthToken, ec: ExecutionContext): setIamPolicy = new setIamPolicy(auth(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/connections/${connectionsId}:setIamPolicy")).addQueryStringParameters("resource" -> resource.toString))
				}
				object repositories {
					class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					...
					class batchCreate(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withBatchCreateRepositoriesRequest(body: Schema.BatchCreateRepositoriesRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Operation])
					}
					object batchCreate {
						def apply(projectsId :PlayApi, locationsId :PlayApi, connectionsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): batchCreate = new batchCreate(auth(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/connections/${connectionsId}/repositories:batchCreate")).addQueryStringParameters("parent" -> parent.toString))
					}
					...
					object accessReadWriteToken {
						def apply(projectsId :PlayApi, locationsId :PlayApi, connectionsId :PlayApi, repositoriesId :PlayApi, repository: String)(using auth: AuthToken, ec: ExecutionContext): accessReadWriteToken = new accessReadWriteToken(auth(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/connections/${connectionsId}/repositories/${repositoriesId}:accessReadWriteToken")).addQueryStringParameters("repository" -> repository.toString))
					}
					class delete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Operation]) {
						def apply() = req.execute("DELETE").map(_.json.as[Schema.Operation])
					}
