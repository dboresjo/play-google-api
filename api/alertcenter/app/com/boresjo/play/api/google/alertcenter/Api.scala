...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object alerts {
		class batchUndelete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withBatchUndeleteAlertsRequest(body: Schema.BatchUndeleteAlertsRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.BatchUndeleteAlertsResponse])
		}
		object batchUndelete {
			def apply()(using auth: AuthToken, ec: ExecutionContext): batchUndelete = new batchUndelete(auth(ws.url(BASE_URL + s"v1beta1/alerts:batchUndelete")).addQueryStringParameters())
		}
		class batchDelete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withBatchDeleteAlertsRequest(body: Schema.BatchDeleteAlertsRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.BatchDeleteAlertsResponse])
		}
		object batchDelete {
			def apply()(using auth: AuthToken, ec: ExecutionContext): batchDelete = new batchDelete(auth(ws.url(BASE_URL + s"v1beta1/alerts:batchDelete")).addQueryStringParameters())
		}
		...
		object delete {
			def apply(alertId: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"v1beta1/alerts/${alertId}")).addQueryStringParameters())
		...
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Alert]) {
			...
			def apply() = req.execute("GET").map(_.json.as[Schema.Alert])
		}
		object get {
			def apply(alertId: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v1beta1/alerts/${alertId}")).addQueryStringParameters())
		...
		object list {
			def apply()(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1beta1/alerts")).addQueryStringParameters())
		...
		class undelete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withUndeleteAlertRequest(body: Schema.UndeleteAlertRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Alert])
		}
		object undelete {
			def apply(alertId: String)(using auth: AuthToken, ec: ExecutionContext): undelete = new undelete(auth(ws.url(BASE_URL + s"v1beta1/alerts/${alertId}:undelete")).addQueryStringParameters())
		}
		class getMetadata(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.AlertMetadata]) {
			...
			def apply() = req.execute("GET").map(_.json.as[Schema.AlertMetadata])
		}
		object getMetadata {
			def apply(alertId: String)(using auth: AuthToken, ec: ExecutionContext): getMetadata = new getMetadata(auth(ws.url(BASE_URL + s"v1beta1/alerts/${alertId}/metadata")).addQueryStringParameters())
		...
		object feedback {
			class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				...
				def withAlertFeedback(body: Schema.AlertFeedback) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.AlertFeedback])
			}
			object create {
				def apply(alertId: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(auth(ws.url(BASE_URL + s"v1beta1/alerts/${alertId}/feedback")).addQueryStringParameters())
			}
			class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListAlertFeedbackResponse]) {
				...
				def withFilter(filter: String) = new list(req.addQueryStringParameters("filter" -> filter.toString))
				def apply() = req.execute("GET").map(_.json.as[Schema.ListAlertFeedbackResponse])
			}
			object list {
				def apply(alertId: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1beta1/alerts/${alertId}/feedback")).addQueryStringParameters())
	...
	object v1beta1 {
		...
		object getSettings {
			def apply()(using auth: AuthToken, ec: ExecutionContext): getSettings = new getSettings(auth(ws.url(BASE_URL + s"v1beta1/settings")).addQueryStringParameters())
		...
		object updateSettings {
			def apply()(using auth: AuthToken, ec: ExecutionContext): updateSettings = new updateSettings(auth(ws.url(BASE_URL + s"v1beta1/settings")).addQueryStringParameters())
		}
	}
}
