...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object tasklists {
		class insert(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withTaskList(body: Schema.TaskList) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.TaskList])
		}
		object insert {
			def apply()(using auth: AuthToken, ec: ExecutionContext): insert = new insert(auth(ws.url(BASE_URL + s"tasks/v1/users/@me/lists")).addQueryStringParameters())
		}
		...
		object delete {
			def apply(tasklist: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"tasks/v1/users/@me/lists/${tasklist}")).addQueryStringParameters())
		...
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.TaskList]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.TaskList])
		}
		object get {
			def apply(tasklist: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"tasks/v1/users/@me/lists/${tasklist}")).addQueryStringParameters())
			given Conversion[get, Future[Schema.TaskList]] = (fun: get) => fun.apply()
		}
		class update(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withTaskList(body: Schema.TaskList) = req.withBody(Json.toJson(body)).execute("PUT").map(_.json.as[Schema.TaskList])
		}
		object update {
			def apply(tasklist: String)(using auth: AuthToken, ec: ExecutionContext): update = new update(auth(ws.url(BASE_URL + s"tasks/v1/users/@me/lists/${tasklist}")).addQueryStringParameters())
		}
		class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withTaskList(body: Schema.TaskList) = req.withBody(Json.toJson(body)).execute("PATCH").map(_.json.as[Schema.TaskList])
		}
		object patch {
			def apply(tasklist: String)(using auth: AuthToken, ec: ExecutionContext): patch = new patch(auth(ws.url(BASE_URL + s"tasks/v1/users/@me/lists/${tasklist}")).addQueryStringParameters())
		}
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.TaskLists]) {
			...
			def apply() = req.execute("GET").map(_.json.as[Schema.TaskLists])
		}
		object list {
			def apply()(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"tasks/v1/users/@me/lists")).addQueryStringParameters())
	...
	object tasks {
		class move(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Task]) {
			...
			def apply() = req.execute("POST").map(_.json.as[Schema.Task])
		}
		object move {
			def apply(tasklist: String, task: String)(using auth: AuthToken, ec: ExecutionContext): move = new move(auth(ws.url(BASE_URL + s"tasks/v1/lists/${tasklist}/tasks/${task}/move")).addQueryStringParameters())
		...
		class insert(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			...
			def withTask(body: Schema.Task) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Task])
		}
		object insert {
			def apply(tasklist: String)(using auth: AuthToken, ec: ExecutionContext): insert = new insert(auth(ws.url(BASE_URL + s"tasks/v1/lists/${tasklist}/tasks")).addQueryStringParameters())
		}
		...
		object clear {
			def apply(tasklist: String)(using auth: AuthToken, ec: ExecutionContext): clear = new clear(auth(ws.url(BASE_URL + s"tasks/v1/lists/${tasklist}/clear")).addQueryStringParameters())
		...
		object delete {
			def apply(tasklist: String, task: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"tasks/v1/lists/${tasklist}/tasks/${task}")).addQueryStringParameters())
			given Conversion[delete, Future[Unit]] = (fun: delete) => fun.apply()
		}
		...
		object get {
			def apply(tasklist: String, task: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"tasks/v1/lists/${tasklist}/tasks/${task}")).addQueryStringParameters())
			given Conversion[get, Future[Schema.Task]] = (fun: get) => fun.apply()
		}
		class update(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withTask(body: Schema.Task) = req.withBody(Json.toJson(body)).execute("PUT").map(_.json.as[Schema.Task])
		}
		object update {
			def apply(task: String, tasklist: String)(using auth: AuthToken, ec: ExecutionContext): update = new update(auth(ws.url(BASE_URL + s"tasks/v1/lists/${tasklist}/tasks/${task}")).addQueryStringParameters())
		}
		class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withTask(body: Schema.Task) = req.withBody(Json.toJson(body)).execute("PATCH").map(_.json.as[Schema.Task])
		}
		object patch {
			def apply(task: String, tasklist: String)(using auth: AuthToken, ec: ExecutionContext): patch = new patch(auth(ws.url(BASE_URL + s"tasks/v1/lists/${tasklist}/tasks/${task}")).addQueryStringParameters())
		}
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Tasks]) {
			...
			def apply() = req.execute("GET").map(_.json.as[Schema.Tasks])
		}
		object list {
			def apply(tasklist: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"tasks/v1/lists/${tasklist}/tasks")).addQueryStringParameters())
