...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object projects {
		class enrollDataSources(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withEnrollDataSourcesRequest(body: Schema.EnrollDataSourcesRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Empty])
		}
		object enrollDataSources {
			def apply(projectsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): enrollDataSources = new enrollDataSources(ws.url(BASE_URL + s"v1/projects/${projectsId}:enrollDataSources").addQueryStringParameters("name" -> name.toString))
		}
		object dataSources {
			class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.DataSource]) {
				def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.DataSource])
			}
			object get {
				def apply(projectsId :PlayApi, dataSourcesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/projects/${projectsId}/dataSources/${dataSourcesId}").addQueryStringParameters("name" -> name.toString))
			...
			class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListDataSourcesResponse]) {
				def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListDataSourcesResponse])
			}
			object list {
				def apply(projectsId :PlayApi, parent: String, pageToken: String, pageSize: Int)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v1/projects/${projectsId}/dataSources").addQueryStringParameters("parent" -> parent.toString, "pageToken" -> pageToken.toString, "pageSize" -> pageSize.toString))
			...
			class checkValidCreds(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				def withCheckValidCredsRequest(body: Schema.CheckValidCredsRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.CheckValidCredsResponse])
			}
			object checkValidCreds {
				def apply(projectsId :PlayApi, dataSourcesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): checkValidCreds = new checkValidCreds(ws.url(BASE_URL + s"v1/projects/${projectsId}/dataSources/${dataSourcesId}:checkValidCreds").addQueryStringParameters("name" -> name.toString))
			}
		}
		object transferConfigs {
			class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				...
				def withTransferConfig(body: Schema.TransferConfig) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.TransferConfig])
			}
			object create {
				def apply(projectsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v1/projects/${projectsId}/transferConfigs").addQueryStringParameters("parent" -> parent.toString))
			}
			class scheduleRuns(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				def withScheduleTransferRunsRequest(body: Schema.ScheduleTransferRunsRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.ScheduleTransferRunsResponse])
			}
			object scheduleRuns {
				def apply(projectsId :PlayApi, transferConfigsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): scheduleRuns = new scheduleRuns(ws.url(BASE_URL + s"v1/projects/${projectsId}/transferConfigs/${transferConfigsId}:scheduleRuns").addQueryStringParameters("parent" -> parent.toString))
			}
			class delete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Empty]) {
				def apply() = auth.exec(req,_.execute("DELETE")).map(_.json.as[Schema.Empty])
			}
			object delete {
				def apply(projectsId :PlayApi, transferConfigsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v1/projects/${projectsId}/transferConfigs/${transferConfigsId}").addQueryStringParameters("name" -> name.toString))
			...
			class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.TransferConfig]) {
				def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.TransferConfig])
			}
			object get {
				def apply(projectsId :PlayApi, transferConfigsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/projects/${projectsId}/transferConfigs/${transferConfigsId}").addQueryStringParameters("name" -> name.toString))
			...
			class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				...
				def withTransferConfig(body: Schema.TransferConfig) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.TransferConfig])
			}
			object patch {
				def apply(projectsId :PlayApi, transferConfigsId :PlayApi, name: String, updateMask: String)(using auth: AuthToken, ec: ExecutionContext): patch = new patch(ws.url(BASE_URL + s"v1/projects/${projectsId}/transferConfigs/${transferConfigsId}").addQueryStringParameters("name" -> name.toString, "updateMask" -> updateMask.toString))
			}
			class startManualRuns(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				def withStartManualTransferRunsRequest(body: Schema.StartManualTransferRunsRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.StartManualTransferRunsResponse])
			}
			object startManualRuns {
				def apply(projectsId :PlayApi, transferConfigsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): startManualRuns = new startManualRuns(ws.url(BASE_URL + s"v1/projects/${projectsId}/transferConfigs/${transferConfigsId}:startManualRuns").addQueryStringParameters("parent" -> parent.toString))
			}
			class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListTransferConfigsResponse]) {
				def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListTransferConfigsResponse])
			}
			object list {
			...
			object runs {
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.TransferRun]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.TransferRun])
				}
				object get {
					def apply(projectsId :PlayApi, transferConfigsId :PlayApi, runsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/projects/${projectsId}/transferConfigs/${transferConfigsId}/runs/${runsId}").addQueryStringParameters("name" -> name.toString))
				...
				object delete {
					def apply(projectsId :PlayApi, transferConfigsId :PlayApi, runsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v1/projects/${projectsId}/transferConfigs/${transferConfigsId}/runs/${runsId}").addQueryStringParameters("name" -> name.toString))
					given Conversion[delete, Future[Schema.Empty]] = (fun: delete) => fun.apply()
				}
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListTransferRunsResponse]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListTransferRunsResponse])
				}
				object list {
				...
				object transferLogs {
					class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListTransferLogsResponse]) {
						def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListTransferLogsResponse])
					}
					object list {
		...
		object locations {
			...
			object enrollDataSources {
				def apply(projectsId :PlayApi, locationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): enrollDataSources = new enrollDataSources(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}:enrollDataSources").addQueryStringParameters("name" -> name.toString))
			}
			class unenrollDataSources(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				def withUnenrollDataSourcesRequest(body: Schema.UnenrollDataSourcesRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Empty])
			}
			object unenrollDataSources {
				def apply(projectsId :PlayApi, locationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): unenrollDataSources = new unenrollDataSources(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}:unenrollDataSources").addQueryStringParameters("name" -> name.toString))
			}
			...
			object transferConfigs {
				...
				object create {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/transferConfigs").addQueryStringParameters("parent" -> parent.toString))
				}
				class scheduleRuns(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withScheduleTransferRunsRequest(body: Schema.ScheduleTransferRunsRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.ScheduleTransferRunsResponse])
				}
				...
				object delete {
					def apply(projectsId :PlayApi, locationsId :PlayApi, transferConfigsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/transferConfigs/${transferConfigsId}").addQueryStringParameters("name" -> name.toString))
					given Conversion[delete, Future[Schema.Empty]] = (fun: delete) => fun.apply()
				}
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.TransferConfig]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.TransferConfig])
				...
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, transferConfigsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/transferConfigs/${transferConfigsId}").addQueryStringParameters("name" -> name.toString))
					given Conversion[get, Future[Schema.TransferConfig]] = (fun: get) => fun.apply()
				}
				class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					/** Deprecated: Authorization code was required when `transferConfig.dataSourceId` is 'youtube_channel' but it is no longer used in any data sources. Use `version_info` instead. Optional OAuth2 authorization code to use with this transfer configuration. This is required only if `transferConfig.dataSourceId` is 'youtube_channel' and new credentials are needed, as indicated by `CheckValidCreds`. In order to obtain authorization_code, make a request to the following URL: https://bigquery.cloud.google.com/datatransfer/oauthz/auth?redirect_uri=urn:ietf:wg:oauth:2.0:oob&response_type=authorization_code&client_id=client_id&scope=data_source_scopes &#42; The client_id is the OAuth client_id of the data source as returned by ListDataSources method. &#42; data_source_scopes are the scopes returned by ListDataSources method. Note that this should not be set when `service_account_name` is used to update the transfer config. */
			...
			object dataSources {
				...
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, dataSourcesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/dataSources/${dataSourcesId}").addQueryStringParameters("name" -> name.toString))
					given Conversion[get, Future[Schema.DataSource]] = (fun: get) => fun.apply()
				}
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListDataSourcesResponse]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListDataSourcesResponse])
