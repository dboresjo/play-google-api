...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object cases {
		class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withCase(body: Schema.Case) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Case])
		}
		object create {
			def apply(v2Id :PlayApi, v2Id1 :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v2/${v2Id}/${v2Id1}/cases").addQueryStringParameters("parent" -> parent.toString))
		}
		class escalate(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withEscalateCaseRequest(body: Schema.EscalateCaseRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Case])
		}
		object escalate {
			def apply(v2Id :PlayApi, v2Id1 :PlayApi, casesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): escalate = new escalate(ws.url(BASE_URL + s"v2/${v2Id}/${v2Id1}/cases/${casesId}:escalate").addQueryStringParameters("name" -> name.toString))
		}
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Case]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.Case])
		}
		object get {
			def apply(v2Id :PlayApi, v2Id1 :PlayApi, casesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v2/${v2Id}/${v2Id1}/cases/${casesId}").addQueryStringParameters("name" -> name.toString))
			given Conversion[get, Future[Schema.Case]] = (fun: get) => fun.apply()
		}
		class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withCase(body: Schema.Case) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.Case])
		}
		object patch {
			def apply(v2Id :PlayApi, v2Id1 :PlayApi, casesId :PlayApi, name: String, updateMask: String)(using auth: AuthToken, ec: ExecutionContext): patch = new patch(ws.url(BASE_URL + s"v2/${v2Id}/${v2Id1}/cases/${casesId}").addQueryStringParameters("name" -> name.toString, "updateMask" -> updateMask.toString))
		}
		class close(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withCloseCaseRequest(body: Schema.CloseCaseRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Case])
		}
		object close {
			def apply(v2Id :PlayApi, v2Id1 :PlayApi, casesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): close = new close(ws.url(BASE_URL + s"v2/${v2Id}/${v2Id1}/cases/${casesId}:close").addQueryStringParameters("name" -> name.toString))
		}
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListCasesResponse]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListCasesResponse])
		}
		object list {
		...
		class search(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.SearchCasesResponse]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.SearchCasesResponse])
		}
		object search {
		...
		object comments {
			...
			object create {
				def apply(v2Id :PlayApi, v2Id1 :PlayApi, casesId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v2/${v2Id}/${v2Id1}/cases/${casesId}/comments").addQueryStringParameters("parent" -> parent.toString))
			}
			class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListCommentsResponse]) {
				def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListCommentsResponse])
			}
			object list {
	...
	object media {
		...
		object download {
			def apply(v2Id :PlayApi, v2Id1 :PlayApi, casesId :PlayApi, attachmentsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): download = new download(ws.url(BASE_URL + s"v2/${v2Id}/${v2Id1}/cases/${casesId}/attachments/${attachmentsId}:download").addQueryStringParameters("name" -> name.toString))
		...
		class upload(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withCreateAttachmentRequest(body: Schema.CreateAttachmentRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Attachment])
		}
		object upload {
			def apply(v2Id :PlayApi, v2Id1 :PlayApi, casesId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): upload = new upload(ws.url(BASE_URL + s"v2/${v2Id}/${v2Id1}/cases/${casesId}/attachments").addQueryStringParameters("parent" -> parent.toString))
		}
	}
	object caseClassifications {
		class search(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.SearchCaseClassificationsResponse]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.SearchCaseClassificationsResponse])
		}
		object search {
			def apply(pageToken: String, query: String, pageSize: Int)(using auth: AuthToken, ec: ExecutionContext): search = new search(ws.url(BASE_URL + s"v2/caseClassifications:search").addQueryStringParameters("pageToken" -> pageToken.toString, "query" -> query.toString, "pageSize" -> pageSize.toString))
