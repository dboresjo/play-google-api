...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object voices {
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListVoicesResponse]) {
			...
			def apply() = req.execute("GET").map(_.json.as[Schema.ListVoicesResponse])
		}
		object list {
			def apply()(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1/voices")).addQueryStringParameters())
	...
	object text {
		class synthesize(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withSynthesizeSpeechRequest(body: Schema.SynthesizeSpeechRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.SynthesizeSpeechResponse])
		}
		object synthesize {
			def apply()(using auth: AuthToken, ec: ExecutionContext): synthesize = new synthesize(auth(ws.url(BASE_URL + s"v1/text:synthesize")).addQueryStringParameters())
		}
	}
	object projects {
		object locations {
			class synthesizeLongAudio(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				def withSynthesizeLongAudioRequest(body: Schema.SynthesizeLongAudioRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Operation])
			}
			object synthesizeLongAudio {
				def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): synthesizeLongAudio = new synthesizeLongAudio(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}:synthesizeLongAudio")).addQueryStringParameters("parent" -> parent.toString))
			}
			object operations {
