...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object attributes {
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListAttributeMetadataResponse]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListAttributeMetadataResponse])
		}
		object list {
	...
	object locations {
		class updateAttributes(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withAttributes(body: Schema.Attributes) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.Attributes])
		}
		object updateAttributes {
			def apply(locationsId :PlayApi, name: String, attributeMask: String)(using auth: AuthToken, ec: ExecutionContext): updateAttributes = new updateAttributes(ws.url(BASE_URL + s"v1/locations/${locationsId}/attributes").addQueryStringParameters("name" -> name.toString, "attributeMask" -> attributeMask.toString))
		}
		class getAttributes(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Attributes]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.Attributes])
		}
		object getAttributes {
			def apply(locationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): getAttributes = new getAttributes(ws.url(BASE_URL + s"v1/locations/${locationsId}/attributes").addQueryStringParameters("name" -> name.toString))
		...
		object delete {
			def apply(locationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v1/locations/${locationsId}").addQueryStringParameters("name" -> name.toString))
		...
		class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			...
			def withLocation(body: Schema.Location) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.Location])
		}
		object patch {
			def apply(locationsId :PlayApi, name: String, updateMask: String)(using auth: AuthToken, ec: ExecutionContext): patch = new patch(ws.url(BASE_URL + s"v1/locations/${locationsId}").addQueryStringParameters("name" -> name.toString, "updateMask" -> updateMask.toString))
		}
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Location]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.Location])
		}
		object get {
			def apply(locationsId :PlayApi, name: String, readMask: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/locations/${locationsId}").addQueryStringParameters("name" -> name.toString, "readMask" -> readMask.toString))
		...
		class getGoogleUpdated(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleUpdatedLocation]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.GoogleUpdatedLocation])
		}
		object getGoogleUpdated {
			def apply(locationsId :PlayApi, name: String, readMask: String)(using auth: AuthToken, ec: ExecutionContext): getGoogleUpdated = new getGoogleUpdated(ws.url(BASE_URL + s"v1/locations/${locationsId}:getGoogleUpdated").addQueryStringParameters("name" -> name.toString, "readMask" -> readMask.toString))
		...
		object attributes {
			...
			object getGoogleUpdated {
				def apply(locationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): getGoogleUpdated = new getGoogleUpdated(ws.url(BASE_URL + s"v1/locations/${locationsId}/attributes:getGoogleUpdated").addQueryStringParameters("name" -> name.toString))
	...
	object categories {
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListCategoriesResponse]) {
			...
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListCategoriesResponse])
		}
		object list {
			def apply(regionCode: String, languageCode: String, view: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v1/categories").addQueryStringParameters("regionCode" -> regionCode.toString, "languageCode" -> languageCode.toString, "view" -> view.toString))
		...
		class batchGet(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.BatchGetCategoriesResponse]) {
			...
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.BatchGetCategoriesResponse])
		}
		object batchGet {
			def apply(names: String, languageCode: String, view: String)(using auth: AuthToken, ec: ExecutionContext): batchGet = new batchGet(ws.url(BASE_URL + s"v1/categories:batchGet").addQueryStringParameters("names" -> names.toString, "languageCode" -> languageCode.toString, "view" -> view.toString))
	...
	object chains {
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Chain]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.Chain])
		}
		object get {
			def apply(chainsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/chains/${chainsId}").addQueryStringParameters("name" -> name.toString))
		...
		class search(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.SearchChainsResponse]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.SearchChainsResponse])
		}
		object search {
			def apply(chainName: String, pageSize: Int)(using auth: AuthToken, ec: ExecutionContext): search = new search(ws.url(BASE_URL + s"v1/chains:search").addQueryStringParameters("chainName" -> chainName.toString, "pageSize" -> pageSize.toString))
	...
	object googleLocations {
		class search(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withSearchGoogleLocationsRequest(body: Schema.SearchGoogleLocationsRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.SearchGoogleLocationsResponse])
		}
		object search {
			def apply()(using auth: AuthToken, ec: ExecutionContext): search = new search(ws.url(BASE_URL + s"v1/googleLocations:search").addQueryStringParameters())
		}
	}
	object accounts {
		object locations {
			...
			object list {
				def apply(accountsId :PlayApi, parent: String, readMask: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v1/accounts/${accountsId}/locations").addQueryStringParameters("parent" -> parent.toString, "readMask" -> readMask.toString))
			...
			class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				...
				def withLocation(body: Schema.Location) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Location])
			}
			object create {
				def apply(accountsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v1/accounts/${accountsId}/locations").addQueryStringParameters("parent" -> parent.toString))
			}
		}
	}
}
