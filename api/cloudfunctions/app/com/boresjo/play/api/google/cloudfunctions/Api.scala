		...
		object locations {
			...
			object functions {
				...
				class setupFunctionUpgradeConfig(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withSetupFunctionUpgradeConfigRequest(body: Schema.SetupFunctionUpgradeConfigRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				...
				class generateDownloadUrl(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withGenerateDownloadUrlRequest(body: Schema.GenerateDownloadUrlRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.GenerateDownloadUrlResponse])
				}
				...
				class rollbackFunctionUpgradeTraffic(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withRollbackFunctionUpgradeTrafficRequest(body: Schema.RollbackFunctionUpgradeTrafficRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				...
				class generateUploadUrl(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withGenerateUploadUrlRequest(body: Schema.GenerateUploadUrlRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.GenerateUploadUrlResponse])
				}
				object generateUploadUrl {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): generateUploadUrl = new generateUploadUrl(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/functions:generateUploadUrl").addQueryStringParameters("parent" -> parent.toString))
				}
				class abortFunctionUpgrade(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withAbortFunctionUpgradeRequest(body: Schema.AbortFunctionUpgradeRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				...
				object delete {
					def apply(projectsId :PlayApi, locationsId :PlayApi, functionsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/functions/${functionsId}").addQueryStringParameters("name" -> name.toString))
				...
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Function]) {
					...
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.Function])
				}
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, functionsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/functions/${functionsId}").addQueryStringParameters("name" -> name.toString))
					given Conversion[get, Future[Schema.Function]] = (fun: get) => fun.apply()
				}
				class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withFunction(body: Schema.Function) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.Operation])
				}
				object patch {
				...
				class redirectFunctionUpgradeTraffic(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withRedirectFunctionUpgradeTrafficRequest(body: Schema.RedirectFunctionUpgradeTrafficRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				...
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withFunction(body: Schema.Function) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object create {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String, functionId: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/functions").addQueryStringParameters("parent" -> parent.toString, "functionId" -> functionId.toString))
				}
				...
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListFunctionsResponse]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListFunctionsResponse])
				}
				object list {
				...
				class commitFunctionUpgrade(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withCommitFunctionUpgradeRequest(body: Schema.CommitFunctionUpgradeRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
			...
			object runtimes {
				...
				object list {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String, filter: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/runtimes").addQueryStringParameters("parent" -> parent.toString, "filter" -> filter.toString))
