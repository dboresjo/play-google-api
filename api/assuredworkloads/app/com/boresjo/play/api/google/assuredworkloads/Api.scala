		...
		object locations {
			object workloads {
				class analyzeWorkloadMove(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleCloudAssuredworkloadsV1AnalyzeWorkloadMoveResponse]) {
					...
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.GoogleCloudAssuredworkloadsV1AnalyzeWorkloadMoveResponse])
				}
				...
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					...
					def withGoogleCloudAssuredworkloadsV1Workload(body: Schema.GoogleCloudAssuredworkloadsV1Workload) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.GoogleLongrunningOperation])
				}
				object create {
					def apply(organizationsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v1/organizations/${organizationsId}/locations/${locationsId}/workloads").addQueryStringParameters("parent" -> parent.toString))
				}
				class enableResourceMonitoring(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleCloudAssuredworkloadsV1EnableResourceMonitoringResponse]) {
					def apply() = auth.exec(req,_.execute("POST")).map(_.json.as[Schema.GoogleCloudAssuredworkloadsV1EnableResourceMonitoringResponse])
				}
				...
				object delete {
					def apply(organizationsId :PlayApi, locationsId :PlayApi, workloadsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v1/organizations/${organizationsId}/locations/${locationsId}/workloads/${workloadsId}").addQueryStringParameters("name" -> name.toString))
				...
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleCloudAssuredworkloadsV1Workload]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.GoogleCloudAssuredworkloadsV1Workload])
				}
				object get {
					def apply(organizationsId :PlayApi, locationsId :PlayApi, workloadsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/organizations/${organizationsId}/locations/${locationsId}/workloads/${workloadsId}").addQueryStringParameters("name" -> name.toString))
				...
				class restrictAllowedResources(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withGoogleCloudAssuredworkloadsV1RestrictAllowedResourcesRequest(body: Schema.GoogleCloudAssuredworkloadsV1RestrictAllowedResourcesRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.GoogleCloudAssuredworkloadsV1RestrictAllowedResourcesResponse])
				}
				...
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleCloudAssuredworkloadsV1ListWorkloadsResponse]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.GoogleCloudAssuredworkloadsV1ListWorkloadsResponse])
				}
				object list {
				...
				class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					...
					def withGoogleCloudAssuredworkloadsV1Workload(body: Schema.GoogleCloudAssuredworkloadsV1Workload) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.GoogleCloudAssuredworkloadsV1Workload])
				}
				object patch {
					def apply(organizationsId :PlayApi, locationsId :PlayApi, workloadsId :PlayApi, updateMask: String)(using auth: AuthToken, ec: ExecutionContext): patch = new patch(ws.url(BASE_URL + s"v1/organizations/${organizationsId}/locations/${locationsId}/workloads/${workloadsId}").addQueryStringParameters("updateMask" -> updateMask.toString))
				}
				class mutatePartnerPermissions(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withGoogleCloudAssuredworkloadsV1MutatePartnerPermissionsRequest(body: Schema.GoogleCloudAssuredworkloadsV1MutatePartnerPermissionsRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.GoogleCloudAssuredworkloadsV1Workload])
				}
				...
				object violations {
					class acknowledge(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withGoogleCloudAssuredworkloadsV1AcknowledgeViolationRequest(body: Schema.GoogleCloudAssuredworkloadsV1AcknowledgeViolationRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.GoogleCloudAssuredworkloadsV1AcknowledgeViolationResponse])
					}
					object acknowledge {
					...
					class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleCloudAssuredworkloadsV1ListViolationsResponse]) {
						...
						def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.GoogleCloudAssuredworkloadsV1ListViolationsResponse])
					}
					object list {
					...
					class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleCloudAssuredworkloadsV1Violation]) {
						def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.GoogleCloudAssuredworkloadsV1Violation])
					}
					object get {
