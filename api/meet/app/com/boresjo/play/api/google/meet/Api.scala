...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object conferenceRecords {
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ConferenceRecord]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.ConferenceRecord])
		}
		object get {
			def apply(conferenceRecordsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v2/conferenceRecords/${conferenceRecordsId}")).addQueryStringParameters("name" -> name.toString))
		...
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListConferenceRecordsResponse]) {
			...
			def apply() = req.execute("GET").map(_.json.as[Schema.ListConferenceRecordsResponse])
		}
		object list {
			def apply()(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v2/conferenceRecords")).addQueryStringParameters())
		...
		object participants {
			class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListParticipantsResponse]) {
				...
				def withFilter(filter: String) = new list(req.addQueryStringParameters("filter" -> filter.toString))
				def apply() = req.execute("GET").map(_.json.as[Schema.ListParticipantsResponse])
			}
			object list {
				def apply(conferenceRecordsId :PlayApi, pageSize: Int, parent: String, pageToken: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v2/conferenceRecords/${conferenceRecordsId}/participants")).addQueryStringParameters("pageSize" -> pageSize.toString, "parent" -> parent.toString, "pageToken" -> pageToken.toString))
			...
			class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Participant]) {
				def apply() = req.execute("GET").map(_.json.as[Schema.Participant])
			}
			object get {
				def apply(conferenceRecordsId :PlayApi, participantsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v2/conferenceRecords/${conferenceRecordsId}/participants/${participantsId}")).addQueryStringParameters("name" -> name.toString))
			...
			object participantSessions {
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ParticipantSession]) {
					def apply() = req.execute("GET").map(_.json.as[Schema.ParticipantSession])
				}
				object get {
					def apply(conferenceRecordsId :PlayApi, participantsId :PlayApi, participantSessionsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v2/conferenceRecords/${conferenceRecordsId}/participants/${participantsId}/participantSessions/${participantSessionsId}")).addQueryStringParameters("name" -> name.toString))
				...
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListParticipantSessionsResponse]) {
					...
					/** Optional. Page token returned from previous List Call. */
					def withPageToken(pageToken: String) = new list(req.addQueryStringParameters("pageToken" -> pageToken.toString))
					def apply() = req.execute("GET").map(_.json.as[Schema.ListParticipantSessionsResponse])
				}
				object list {
					def apply(conferenceRecordsId :PlayApi, participantsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v2/conferenceRecords/${conferenceRecordsId}/participants/${participantsId}/participantSessions")).addQueryStringParameters("parent" -> parent.toString))
		...
		object transcripts {
			class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Transcript]) {
				def apply() = req.execute("GET").map(_.json.as[Schema.Transcript])
			}
			object get {
				def apply(conferenceRecordsId :PlayApi, transcriptsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v2/conferenceRecords/${conferenceRecordsId}/transcripts/${transcriptsId}")).addQueryStringParameters("name" -> name.toString))
			...
			class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListTranscriptsResponse]) {
				def apply() = req.execute("GET").map(_.json.as[Schema.ListTranscriptsResponse])
			}
			object list {
				def apply(conferenceRecordsId :PlayApi, pageToken: String, parent: String, pageSize: Int)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v2/conferenceRecords/${conferenceRecordsId}/transcripts")).addQueryStringParameters("pageToken" -> pageToken.toString, "parent" -> parent.toString, "pageSize" -> pageSize.toString))
			...
			object entries {
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.TranscriptEntry]) {
					def apply() = req.execute("GET").map(_.json.as[Schema.TranscriptEntry])
				}
				object get {
					def apply(conferenceRecordsId :PlayApi, transcriptsId :PlayApi, entriesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v2/conferenceRecords/${conferenceRecordsId}/transcripts/${transcriptsId}/entries/${entriesId}")).addQueryStringParameters("name" -> name.toString))
		...
		object recordings {
			class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Recording]) {
				def apply() = req.execute("GET").map(_.json.as[Schema.Recording])
			}
			object get {
				def apply(conferenceRecordsId :PlayApi, recordingsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v2/conferenceRecords/${conferenceRecordsId}/recordings/${recordingsId}")).addQueryStringParameters("name" -> name.toString))
			...
			class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListRecordingsResponse]) {
				def apply() = req.execute("GET").map(_.json.as[Schema.ListRecordingsResponse])
			}
			object list {
				def apply(conferenceRecordsId :PlayApi, pageToken: String, pageSize: Int, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v2/conferenceRecords/${conferenceRecordsId}/recordings")).addQueryStringParameters("pageToken" -> pageToken.toString, "pageSize" -> pageSize.toString, "parent" -> parent.toString))
	...
	object spaces {
		...
		object create {
			def apply()(using auth: AuthToken, ec: ExecutionContext): create = new create(auth(ws.url(BASE_URL + s"v2/spaces")).addQueryStringParameters())
		}
		...
		object get {
			def apply(spacesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v2/spaces/${spacesId}")).addQueryStringParameters("name" -> name.toString))
		...
		class endActiveConference(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withEndActiveConferenceRequest(body: Schema.EndActiveConferenceRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Empty])
		}
		object endActiveConference {
			def apply(spacesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): endActiveConference = new endActiveConference(auth(ws.url(BASE_URL + s"v2/spaces/${spacesId}:endActiveConference")).addQueryStringParameters("name" -> name.toString))
		}
		...
		object patch {
			def apply(spacesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): patch = new patch(auth(ws.url(BASE_URL + s"v2/spaces/${spacesId}")).addQueryStringParameters("name" -> name.toString))
		}
	}
}
