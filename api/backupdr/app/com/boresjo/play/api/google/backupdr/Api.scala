		...
		object locations {
			...
			object backupPlans {
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					...
					def withBackupPlan(body: Schema.BackupPlan) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object create {
			...
			object backupVaults {
				...
				class fetchUsable(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.FetchUsableBackupVaultsResponse]) {
					...
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.FetchUsableBackupVaultsResponse])
				}
				object fetchUsable {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): fetchUsable = new fetchUsable(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/backupVaults:fetchUsable").addQueryStringParameters("parent" -> parent.toString))
				...
				object list {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/backupVaults").addQueryStringParameters("parent" -> parent.toString))
				...
				object dataSources {
					class setInternalStatus(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withSetInternalStatusRequest(body: Schema.SetInternalStatusRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
					}
					...
					class finalizeBackup(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withFinalizeBackupRequest(body: Schema.FinalizeBackupRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
					}
					...
					class fetchAccessToken(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withFetchAccessTokenRequest(body: Schema.FetchAccessTokenRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.FetchAccessTokenResponse])
					}
					...
					class remove(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withRemoveDataSourceRequest(body: Schema.RemoveDataSourceRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
					}
					object remove {
					...
					class abandonBackup(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withAbandonBackupRequest(body: Schema.AbandonBackupRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
					}
					...
					class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						...
						def withDataSource(body: Schema.DataSource) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.Operation])
					}
					object patch {
					...
					object list {
						def apply(projectsId :PlayApi, locationsId :PlayApi, backupVaultsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/backupVaults/${backupVaultsId}/dataSources").addQueryStringParameters("parent" -> parent.toString))
					...
					class initiateBackup(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withInitiateBackupRequest(body: Schema.InitiateBackupRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.InitiateBackupResponse])
					}
					...
					object backups {
						class restore(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
							def withRestoreBackupRequest(body: Schema.RestoreBackupRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
						}
						object restore {
			...
			object managementServers {
				...
				object delete {
					def apply(projectsId :PlayApi, locationsId :PlayApi, managementServersId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/managementServers/${managementServersId}").addQueryStringParameters("name" -> name.toString))
					given Conversion[delete, Future[Schema.Operation]] = (fun: delete) => fun.apply()
				}
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ManagementServer]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ManagementServer])
				}
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, managementServersId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/managementServers/${managementServersId}").addQueryStringParameters("name" -> name.toString))
				...
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListManagementServersResponse]) {
					...
					/** Optional. Hint for how to order the results. */
					def withOrderBy(orderBy: String) = new list(req.addQueryStringParameters("orderBy" -> orderBy.toString))
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListManagementServersResponse])
				}
				object list {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/managementServers").addQueryStringParameters("parent" -> parent.toString))
					given Conversion[list, Future[Schema.ListManagementServersResponse]] = (fun: list) => fun.apply()
				}
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					/** Optional. An optional request ID to identify requests. Specify a unique request ID so that if you must retry your request, the server will know to ignore the request if it has already been completed. The server will guarantee that for at least 60 minutes since the first request. For example, consider a situation where you make an initial request and the request times out. If you make the request again with the same request ID, the server can check if original operation with the same request ID was received, and if so, will ignore the second request. This prevents clients from accidentally creating duplicate commitments. The request ID must be a valid UUID with the exception that zero UUID is not supported (00000000-0000-0000-0000-000000000000). */
					def withRequestId(requestId: String) = new create(req.addQueryStringParameters("requestId" -> requestId.toString))
					def withManagementServer(body: Schema.ManagementServer) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object create {
			...
			object backupPlanAssociations {
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					/** Optional. An optional request ID to identify requests. Specify a unique request ID so that if you must retry your request, the server will know to ignore the request if it has already been completed. The server will guarantee that for at least 60 minutes since the first request. For example, consider a situation where you make an initial request and t he request times out. If you make the request again with the same request ID, the server can check if original operation with the same request ID was received, and if so, will ignore the second request. This prevents clients from accidentally creating duplicate commitments. The request ID must be a valid UUID with the exception that zero UUID is not supported (00000000-0000-0000-0000-000000000000). */
					def withRequestId(requestId: String) = new create(req.addQueryStringParameters("requestId" -> requestId.toString))
					def withBackupPlanAssociation(body: Schema.BackupPlanAssociation) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object create {
				...
				class triggerBackup(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withTriggerBackupRequest(body: Schema.TriggerBackupRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				...
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.BackupPlanAssociation]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.BackupPlanAssociation])
				}
				object get {
				...
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListBackupPlanAssociationsResponse]) {
					...
					/** Optional. Filtering results */
					def withFilter(filter: String) = new list(req.addQueryStringParameters("filter" -> filter.toString))
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListBackupPlanAssociationsResponse])
				}
				object list {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/backupPlanAssociations").addQueryStringParameters("parent" -> parent.toString))
