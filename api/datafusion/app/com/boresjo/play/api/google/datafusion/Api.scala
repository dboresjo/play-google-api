		...
		object locations {
			...
			object versions {
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListAvailableVersionsResponse]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListAvailableVersionsResponse])
				}
				object list {
			...
			object instances {
				...
				object dnsPeerings {
					class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withDnsPeering(body: Schema.DnsPeering) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.DnsPeering])
					}
					object create {
					...
					class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListDnsPeeringsResponse]) {
						def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListDnsPeeringsResponse])
					}
					object list {
