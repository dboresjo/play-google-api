...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object activities {
		...
		object watch {
			def apply(userKey: String, applicationName: String, actorIpAddress: String, customerId: String, endTime: String, eventName: String, filters: String, orgUnitID: String, pageToken: String, startTime: String, groupIdFilter: String)(using auth: AuthToken, ec: ExecutionContext): watch = new watch(auth(ws.url(BASE_URL + s"admin/reports/v1/activity/users/${userKey}/applications/${applicationName}/watch")).addQueryStringParameters("actorIpAddress" -> actorIpAddress.toString, "customerId" -> customerId.toString, "endTime" -> endTime.toString, "eventName" -> eventName.toString, "filters" -> filters.toString, "orgUnitID" -> orgUnitID.toString, "pageToken" -> pageToken.toString, "startTime" -> startTime.toString, "groupIdFilter" -> groupIdFilter.toString))
		}
	}
	object channels {
		...
		object stop {
			def apply()(using auth: AuthToken, ec: ExecutionContext): stop = new stop(auth(ws.url(BASE_URL + s"admin/reports_v1/channels/stop")).addQueryStringParameters())
		}
	}
	object customerUsageReports {
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.UsageReports]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.UsageReports])
		}
		object get {
			def apply(date: String, customerId: String, pageToken: String, parameters: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"admin/reports/v1/usage/dates/${date}")).addQueryStringParameters("customerId" -> customerId.toString, "pageToken" -> pageToken.toString, "parameters" -> parameters.toString))
