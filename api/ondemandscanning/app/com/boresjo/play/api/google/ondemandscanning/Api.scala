		...
		object locations {
			...
			object scans {
				class analyzePackages(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withAnalyzePackagesRequestV1(body: Schema.AnalyzePackagesRequestV1) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Operation])
				}
				object analyzePackages {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): analyzePackages = new analyzePackages(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/scans:analyzePackages")).addQueryStringParameters("parent" -> parent.toString))
				}
				object vulnerabilities {
					class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListVulnerabilitiesResponseV1]) {
						def apply() = req.execute("GET").map(_.json.as[Schema.ListVulnerabilitiesResponseV1])
					}
					object list {
						def apply(projectsId :PlayApi, locationsId :PlayApi, scansId :PlayApi, parent: String, pageSize: Int, pageToken: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/scans/${scansId}/vulnerabilities")).addQueryStringParameters("parent" -> parent.toString, "pageSize" -> pageSize.toString, "pageToken" -> pageToken.toString))
