...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object places {
		class searchNearby(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withGoogleMapsPlacesV1SearchNearbyRequest(body: Schema.GoogleMapsPlacesV1SearchNearbyRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.GoogleMapsPlacesV1SearchNearbyResponse])
		}
		object searchNearby {
			def apply()(using auth: AuthToken, ec: ExecutionContext): searchNearby = new searchNearby(ws.url(BASE_URL + s"v1/places:searchNearby").addQueryStringParameters())
		}
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleMapsPlacesV1Place]) {
			...
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.GoogleMapsPlacesV1Place])
		}
		object get {
			def apply(placesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/places/${placesId}").addQueryStringParameters("name" -> name.toString))
		...
		class searchText(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withGoogleMapsPlacesV1SearchTextRequest(body: Schema.GoogleMapsPlacesV1SearchTextRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.GoogleMapsPlacesV1SearchTextResponse])
		}
		object searchText {
			def apply()(using auth: AuthToken, ec: ExecutionContext): searchText = new searchText(ws.url(BASE_URL + s"v1/places:searchText").addQueryStringParameters())
		}
		class autocomplete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withGoogleMapsPlacesV1AutocompletePlacesRequest(body: Schema.GoogleMapsPlacesV1AutocompletePlacesRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.GoogleMapsPlacesV1AutocompletePlacesResponse])
		}
		object autocomplete {
			def apply()(using auth: AuthToken, ec: ExecutionContext): autocomplete = new autocomplete(ws.url(BASE_URL + s"v1/places:autocomplete").addQueryStringParameters())
		}
		object photos {
			class getMedia(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleMapsPlacesV1PhotoMedia]) {
				...
				def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.GoogleMapsPlacesV1PhotoMedia])
			}
			object getMedia {
				def apply(placesId :PlayApi, photosId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): getMedia = new getMedia(ws.url(BASE_URL + s"v1/places/${placesId}/photos/${photosId}/media").addQueryStringParameters("name" -> name.toString))
