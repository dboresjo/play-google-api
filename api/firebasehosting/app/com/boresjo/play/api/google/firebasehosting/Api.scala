...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	private val BASE_URL = "https://firebasehosting.googleapis.com/"
		...
		object sites {
			object customDomains {
				object operations {
					...
					object delete {
						def apply(projectsId :PlayApi, sitesId :PlayApi, customDomainsId :PlayApi, operationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/sites/${sitesId}/customDomains/${customDomainsId}/operations/${operationsId}")).addQueryStringParameters("name" -> name.toString))
						given Conversion[delete, Future[Schema.Empty]] = (fun: delete) => fun.apply()
					}
					class cancel(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withCancelOperationRequest(body: Schema.CancelOperationRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Empty])
					...
					object cancel {
						def apply(projectsId :PlayApi, sitesId :PlayApi, customDomainsId :PlayApi, operationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): cancel = new cancel(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/sites/${sitesId}/customDomains/${customDomainsId}/operations/${operationsId}:cancel")).addQueryStringParameters("name" -> name.toString))
					}
				}
			}
		}
