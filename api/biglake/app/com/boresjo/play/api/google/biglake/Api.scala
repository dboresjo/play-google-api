		...
		object locations {
			object catalogs {
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withCatalog(body: Schema.Catalog) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Catalog])
				}
				object create {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String, catalogId: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/catalogs")).addQueryStringParameters("parent" -> parent.toString, "catalogId" -> catalogId.toString))
				}
				class delete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Catalog]) {
					def apply() = req.execute("DELETE").map(_.json.as[Schema.Catalog])
				}
				object delete {
					def apply(projectsId :PlayApi, locationsId :PlayApi, catalogsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/catalogs/${catalogsId}")).addQueryStringParameters("name" -> name.toString))
				...
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Catalog]) {
					def apply() = req.execute("GET").map(_.json.as[Schema.Catalog])
				}
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, catalogsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/catalogs/${catalogsId}")).addQueryStringParameters("name" -> name.toString))
				...
				object databases {
					...
					object get {
						def apply(projectsId :PlayApi, locationsId :PlayApi, catalogsId :PlayApi, databasesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/catalogs/${catalogsId}/databases/${databasesId}")).addQueryStringParameters("name" -> name.toString))
						given Conversion[get, Future[Schema.Database]] = (fun: get) => fun.apply()
					}
					class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withDatabase(body: Schema.Database) = req.withBody(Json.toJson(body)).execute("PATCH").map(_.json.as[Schema.Database])
					}
					object patch {
						def apply(projectsId :PlayApi, locationsId :PlayApi, catalogsId :PlayApi, databasesId :PlayApi, name: String, updateMask: String)(using auth: AuthToken, ec: ExecutionContext): patch = new patch(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/catalogs/${catalogsId}/databases/${databasesId}")).addQueryStringParameters("name" -> name.toString, "updateMask" -> updateMask.toString))
					}
					...
					object tables {
						...
						class rename(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
							def withRenameTableRequest(body: Schema.RenameTableRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Table])
						}
						object rename {
							def apply(projectsId :PlayApi, locationsId :PlayApi, catalogsId :PlayApi, databasesId :PlayApi, tablesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): rename = new rename(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/catalogs/${catalogsId}/databases/${databasesId}/tables/${tablesId}:rename")).addQueryStringParameters("name" -> name.toString))
						}
						class delete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Table]) {
							def apply() = req.execute("DELETE").map(_.json.as[Schema.Table])
						}
						object delete {
