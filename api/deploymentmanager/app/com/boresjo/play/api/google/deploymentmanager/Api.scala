...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object deployments {
		class testIamPermissions(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withTestPermissionsRequest(body: Schema.TestPermissionsRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.TestPermissionsResponse])
		}
		...
		class setIamPolicy(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withGlobalSetPolicyRequest(body: Schema.GlobalSetPolicyRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Policy])
		}
		object setIamPolicy {
			def apply(project: String, resource: String)(using auth: AuthToken, ec: ExecutionContext): setIamPolicy = new setIamPolicy(ws.url(BASE_URL + s"deploymentmanager/v2/projects/${project}/global/deployments/${resource}/setIamPolicy").addQueryStringParameters())
		}
		class stop(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withDeploymentsStopRequest(body: Schema.DeploymentsStopRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
		}
		object stop {
			def apply(project: String, deployment: String)(using auth: AuthToken, ec: ExecutionContext): stop = new stop(ws.url(BASE_URL + s"deploymentmanager/v2/projects/${project}/global/deployments/${deployment}/stop").addQueryStringParameters())
		}
		...
		class update(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withDeployment(body: Schema.Deployment) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PUT")).map(_.json.as[Schema.Operation])
		}
		object update {
		...
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.DeploymentsListResponse]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.DeploymentsListResponse])
		}
		object list {
		...
		class cancelPreview(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withDeploymentsCancelPreviewRequest(body: Schema.DeploymentsCancelPreviewRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
		}
		object cancelPreview {
			def apply(project: String, deployment: String)(using auth: AuthToken, ec: ExecutionContext): cancelPreview = new cancelPreview(ws.url(BASE_URL + s"deploymentmanager/v2/projects/${project}/global/deployments/${deployment}/cancelPreview").addQueryStringParameters())
		}
	}
	object manifests {
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Manifest]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.Manifest])
		}
		object get {
		...
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ManifestsListResponse]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ManifestsListResponse])
		}
		object list {
	...
	object operations {
		...
		object get {
			def apply(project: String, operation: String, headerBypassBillingFilter: Boolean)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"deploymentmanager/v2/projects/${project}/global/operations/${operation}").addQueryStringParameters("header.bypassBillingFilter" -> headerBypassBillingFilter.toString))
	...
	object resources {
		...
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ResourcesListResponse]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ResourcesListResponse])
		}
		object list {
	...
	object types {
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.TypesListResponse]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.TypesListResponse])
		}
		object list {
