	...
	object projects {
		object cryptoKeys {
			class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleCloudKmsInventoryV1ListCryptoKeysResponse]) {
				...
				def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.GoogleCloudKmsInventoryV1ListCryptoKeysResponse])
			}
			object list {
				def apply(projectsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v1/projects/${projectsId}/cryptoKeys").addQueryStringParameters("parent" -> parent.toString))
			...
			object keyRings {
				object cryptoKeys {
					class getProtectedResourcesSummary(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleCloudKmsInventoryV1ProtectedResourcesSummary]) {
						def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.GoogleCloudKmsInventoryV1ProtectedResourcesSummary])
					}
	...
	object organizations {
		object protectedResources {
			class search(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleCloudKmsInventoryV1SearchProtectedResourcesResponse]) {
				...
				def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.GoogleCloudKmsInventoryV1SearchProtectedResourcesResponse])
			}
			object search {
