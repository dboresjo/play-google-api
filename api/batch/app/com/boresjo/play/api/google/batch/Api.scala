		...
		object locations {
			...
			object state {
				class report(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withReportAgentStateRequest(body: Schema.ReportAgentStateRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.ReportAgentStateResponse])
				}
				object report {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): report = new report(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/state:report")).addQueryStringParameters("parent" -> parent.toString))
				}
			}
			object jobs {
				...
				object create {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String, jobId: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/jobs")).addQueryStringParameters("parent" -> parent.toString, "jobId" -> jobId.toString))
				}
				...
				object taskGroups {
					object tasks {
						...
						object list {
							def apply(projectsId :PlayApi, locationsId :PlayApi, jobsId :PlayApi, taskGroupsId :PlayApi, parent: String, filter: String, pageSize: Int, pageToken: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/jobs/${jobsId}/taskGroups/${taskGroupsId}/tasks")).addQueryStringParameters("parent" -> parent.toString, "filter" -> filter.toString, "pageSize" -> pageSize.toString, "pageToken" -> pageToken.toString))
