...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object hashes {
		class search(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleCloudWebriskV1SearchHashesResponse]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.GoogleCloudWebriskV1SearchHashesResponse])
		}
		object search {
			def apply(threatTypes: String, hashPrefix: String)(using auth: AuthToken, ec: ExecutionContext): search = new search(auth(ws.url(BASE_URL + s"v1/hashes:search")).addQueryStringParameters("threatTypes" -> threatTypes.toString, "hashPrefix" -> hashPrefix.toString))
	...
	object uris {
		class search(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleCloudWebriskV1SearchUrisResponse]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.GoogleCloudWebriskV1SearchUrisResponse])
		}
		object search {
			def apply(threatTypes: String, uri: String)(using auth: AuthToken, ec: ExecutionContext): search = new search(auth(ws.url(BASE_URL + s"v1/uris:search")).addQueryStringParameters("threatTypes" -> threatTypes.toString, "uri" -> uri.toString))
	...
	object projects {
		object submissions {
			class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				def withGoogleCloudWebriskV1Submission(body: Schema.GoogleCloudWebriskV1Submission) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.GoogleCloudWebriskV1Submission])
			}
			object create {
				def apply(projectsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/submissions")).addQueryStringParameters("parent" -> parent.toString))
			}
		}
		object operations {
			...
			object list {
				def apply(projectsId :PlayApi, pageToken: String, pageSize: Int, filter: String, name: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/operations")).addQueryStringParameters("pageToken" -> pageToken.toString, "pageSize" -> pageSize.toString, "filter" -> filter.toString, "name" -> name.toString))
