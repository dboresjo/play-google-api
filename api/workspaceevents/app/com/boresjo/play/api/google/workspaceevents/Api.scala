...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object subscriptions {
		class reactivate(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withReactivateSubscriptionRequest(body: Schema.ReactivateSubscriptionRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Operation])
		}
		object reactivate {
			def apply(subscriptionsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): reactivate = new reactivate(auth(ws.url(BASE_URL + s"v1/subscriptions/${subscriptionsId}:reactivate")).addQueryStringParameters("name" -> name.toString))
		}
		class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			...
			def withSubscription(body: Schema.Subscription) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Operation])
		}
		object create {
			def apply()(using auth: AuthToken, ec: ExecutionContext): create = new create(auth(ws.url(BASE_URL + s"v1/subscriptions")).addQueryStringParameters())
		}
		...
		object delete {
			def apply(subscriptionsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"v1/subscriptions/${subscriptionsId}")).addQueryStringParameters("name" -> name.toString))
		...
		object get {
			def apply(subscriptionsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v1/subscriptions/${subscriptionsId}")).addQueryStringParameters("name" -> name.toString))
		...
		class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			...
			def withSubscription(body: Schema.Subscription) = req.withBody(Json.toJson(body)).execute("PATCH").map(_.json.as[Schema.Operation])
		}
		object patch {
			def apply(subscriptionsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): patch = new patch(auth(ws.url(BASE_URL + s"v1/subscriptions/${subscriptionsId}")).addQueryStringParameters("name" -> name.toString))
		}
		...
		object list {
			def apply(filter: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1/subscriptions")).addQueryStringParameters("filter" -> filter.toString))
