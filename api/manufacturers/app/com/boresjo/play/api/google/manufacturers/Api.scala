...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object accounts {
		object products {
			...
			object get {
				def apply(accountsId :PlayApi, productsId :PlayApi, parent: String, name: String, include: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v1/accounts/${accountsId}/products/${productsId}")).addQueryStringParameters("parent" -> parent.toString, "name" -> name.toString, "include" -> include.toString))
				given Conversion[get, Future[Schema.Product]] = (fun: get) => fun.apply()
			}
			class update(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				def withAttributes(body: Schema.Attributes) = req.withBody(Json.toJson(body)).execute("PUT").map(_.json.as[Schema.Empty])
			}
			object update {
				def apply(accountsId :PlayApi, productsId :PlayApi, parent: String, name: String)(using auth: AuthToken, ec: ExecutionContext): update = new update(auth(ws.url(BASE_URL + s"v1/accounts/${accountsId}/products/${productsId}")).addQueryStringParameters("parent" -> parent.toString, "name" -> name.toString))
			}
			class delete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Empty]) {
				def apply() = req.execute("DELETE").map(_.json.as[Schema.Empty])
			}
			object delete {
				def apply(accountsId :PlayApi, productsId :PlayApi, parent: String, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"v1/accounts/${accountsId}/products/${productsId}")).addQueryStringParameters("parent" -> parent.toString, "name" -> name.toString))
		...
		object languages {
			object productCertifications {
				class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					...
					def withProductCertification(body: Schema.ProductCertification) = req.withBody(Json.toJson(body)).execute("PATCH").map(_.json.as[Schema.ProductCertification])
				}
				object patch {
					def apply(accountsId :PlayApi, languagesId :PlayApi, productCertificationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): patch = new patch(auth(ws.url(BASE_URL + s"v1/accounts/${accountsId}/languages/${languagesId}/productCertifications/${productCertificationsId}")).addQueryStringParameters("name" -> name.toString))
				}
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListProductCertificationsResponse]) {
					...
					def apply() = req.execute("GET").map(_.json.as[Schema.ListProductCertificationsResponse])
				}
				object list {
					def apply(accountsId :PlayApi, languagesId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1/accounts/${accountsId}/languages/${languagesId}/productCertifications")).addQueryStringParameters("parent" -> parent.toString))
				...
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ProductCertification]) {
					def apply() = req.execute("GET").map(_.json.as[Schema.ProductCertification])
				}
				object get {
					def apply(accountsId :PlayApi, languagesId :PlayApi, productCertificationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v1/accounts/${accountsId}/languages/${languagesId}/productCertifications/${productCertificationsId}")).addQueryStringParameters("name" -> name.toString))
				...
				object delete {
					def apply(accountsId :PlayApi, languagesId :PlayApi, productCertificationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"v1/accounts/${accountsId}/languages/${languagesId}/productCertifications/${productCertificationsId}")).addQueryStringParameters("name" -> name.toString))
					given Conversion[delete, Future[Schema.Empty]] = (fun: delete) => fun.apply()
				}
			}
		}
