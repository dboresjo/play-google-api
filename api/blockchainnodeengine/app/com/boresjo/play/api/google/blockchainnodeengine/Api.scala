		...
		object locations {
			...
			object blockchainNodes {
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					...
					def withBlockchainNode(body: Schema.BlockchainNode) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object create {
				...
				object delete {
					def apply(projectsId :PlayApi, locationsId :PlayApi, blockchainNodesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/blockchainNodes/${blockchainNodesId}").addQueryStringParameters("name" -> name.toString))
				...
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.BlockchainNode]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.BlockchainNode])
				}
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, blockchainNodesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/blockchainNodes/${blockchainNodesId}").addQueryStringParameters("name" -> name.toString))
				...
				class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					...
					def withBlockchainNode(body: Schema.BlockchainNode) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.Operation])
				}
				object patch {
				...
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListBlockchainNodesResponse]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListBlockchainNodesResponse])
				}
				object list {
