...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object placeActionTypeMetadata {
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListPlaceActionTypeMetadataResponse]) {
			...
			def apply() = req.execute("GET").map(_.json.as[Schema.ListPlaceActionTypeMetadataResponse])
		}
		object list {
			def apply()(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1/placeActionTypeMetadata")).addQueryStringParameters())
	...
	object locations {
		object placeActionLinks {
			class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				def withPlaceActionLink(body: Schema.PlaceActionLink) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.PlaceActionLink])
			}
			object create {
				def apply(locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(auth(ws.url(BASE_URL + s"v1/locations/${locationsId}/placeActionLinks")).addQueryStringParameters("parent" -> parent.toString))
			}
			class delete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Empty]) {
				def apply() = req.execute("DELETE").map(_.json.as[Schema.Empty])
			}
			object delete {
				def apply(locationsId :PlayApi, placeActionLinksId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"v1/locations/${locationsId}/placeActionLinks/${placeActionLinksId}")).addQueryStringParameters("name" -> name.toString))
			...
			class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.PlaceActionLink]) {
				def apply() = req.execute("GET").map(_.json.as[Schema.PlaceActionLink])
			}
			object get {
				def apply(locationsId :PlayApi, placeActionLinksId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v1/locations/${locationsId}/placeActionLinks/${placeActionLinksId}")).addQueryStringParameters("name" -> name.toString))
			...
			class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				...
				def withPlaceActionLink(body: Schema.PlaceActionLink) = req.withBody(Json.toJson(body)).execute("PATCH").map(_.json.as[Schema.PlaceActionLink])
			}
			object patch {
				def apply(locationsId :PlayApi, placeActionLinksId :PlayApi, updateMask: String)(using auth: AuthToken, ec: ExecutionContext): patch = new patch(auth(ws.url(BASE_URL + s"v1/locations/${locationsId}/placeActionLinks/${placeActionLinksId}")).addQueryStringParameters("updateMask" -> updateMask.toString))
			}
			class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListPlaceActionLinksResponse]) {
				...
				def withPageToken(pageToken: String) = new list(req.addQueryStringParameters("pageToken" -> pageToken.toString))
				def apply() = req.execute("GET").map(_.json.as[Schema.ListPlaceActionLinksResponse])
			}
			object list {
				def apply(locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1/locations/${locationsId}/placeActionLinks")).addQueryStringParameters("parent" -> parent.toString))
