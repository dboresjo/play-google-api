	...
	object projects {
		object locations {
			...
			class generateServiceIdentity(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				def withGenerateServiceIdentityRequest(body: Schema.GenerateServiceIdentityRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.GenerateServiceIdentityResponse])
			}
			object generateServiceIdentity {
				def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): generateServiceIdentity = new generateServiceIdentity(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}:generateServiceIdentity").addQueryStringParameters("parent" -> parent.toString))
			}
			object operations {
				...
				object delete {
					def apply(projectsId :PlayApi, locationsId :PlayApi, operationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/operations/${operationsId}").addQueryStringParameters("name" -> name.toString))
			...
			object nodes {
				class getGuestAttributes(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withGetGuestAttributesRequest(body: Schema.GetGuestAttributesRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.GetGuestAttributesResponse])
				}
				object getGuestAttributes {
					def apply(projectsId :PlayApi, locationsId :PlayApi, nodesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): getGuestAttributes = new getGuestAttributes(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/nodes/${nodesId}:getGuestAttributes").addQueryStringParameters("name" -> name.toString))
				}
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withNode(body: Schema.Node) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object create {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String, nodeId: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/nodes").addQueryStringParameters("parent" -> parent.toString, "nodeId" -> nodeId.toString))
				}
				class stop(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withStopNodeRequest(body: Schema.StopNodeRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object stop {
					def apply(projectsId :PlayApi, locationsId :PlayApi, nodesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): stop = new stop(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/nodes/${nodesId}:stop").addQueryStringParameters("name" -> name.toString))
				}
				class delete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Operation]) {
					def apply() = auth.exec(req,_.execute("DELETE")).map(_.json.as[Schema.Operation])
				}
				object delete {
					def apply(projectsId :PlayApi, locationsId :PlayApi, nodesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/nodes/${nodesId}").addQueryStringParameters("name" -> name.toString))
				...
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, nodesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/nodes/${nodesId}").addQueryStringParameters("name" -> name.toString))
				...
				class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withNode(body: Schema.Node) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.Operation])
				}
				object patch {
				...
				class start(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withStartNodeRequest(body: Schema.StartNodeRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object start {
					def apply(projectsId :PlayApi, locationsId :PlayApi, nodesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): start = new start(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/nodes/${nodesId}:start").addQueryStringParameters("name" -> name.toString))
				}
			}
			object runtimeVersions {
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListRuntimeVersionsResponse]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListRuntimeVersionsResponse])
				}
				object list {
				...
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.RuntimeVersion]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.RuntimeVersion])
				}
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, runtimeVersionsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/runtimeVersions/${runtimeVersionsId}").addQueryStringParameters("name" -> name.toString))
			...
			object queuedResources {
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					...
					def withQueuedResource(body: Schema.QueuedResource) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object create {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/queuedResources").addQueryStringParameters("parent" -> parent.toString))
				}
				class delete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Operation]) {
					/** Optional. Idempotent request UUID. */
				...
				object delete {
					def apply(projectsId :PlayApi, locationsId :PlayApi, queuedResourcesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/queuedResources/${queuedResourcesId}").addQueryStringParameters("name" -> name.toString))
					given Conversion[delete, Future[Schema.Operation]] = (fun: delete) => fun.apply()
				}
				class reset(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withResetQueuedResourceRequest(body: Schema.ResetQueuedResourceRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object reset {
					def apply(projectsId :PlayApi, locationsId :PlayApi, queuedResourcesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): reset = new reset(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/queuedResources/${queuedResourcesId}:reset").addQueryStringParameters("name" -> name.toString))
				}
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.QueuedResource]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.QueuedResource])
				}
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, queuedResourcesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/queuedResources/${queuedResourcesId}").addQueryStringParameters("name" -> name.toString))
				...
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListQueuedResourcesResponse]) {
					...
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListQueuedResourcesResponse])
				}
				object list {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/queuedResources").addQueryStringParameters("parent" -> parent.toString))
			...
			object acceleratorTypes {
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListAcceleratorTypesResponse]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListAcceleratorTypesResponse])
				}
				object list {
				...
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.AcceleratorType]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.AcceleratorType])
				}
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, acceleratorTypesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/acceleratorTypes/${acceleratorTypesId}").addQueryStringParameters("name" -> name.toString))
