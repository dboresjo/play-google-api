		...
		object locations {
			...
			object connections {
				...
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					...
					def withConnection(body: Schema.Connection) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Operation])
				}
				object create {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String, connectionId: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/connections")).addQueryStringParameters("parent" -> parent.toString, "connectionId" -> connectionId.toString))
				}
				class processGitHubEnterpriseWebhook(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withProcessGitHubEnterpriseWebhookRequest(body: Schema.ProcessGitHubEnterpriseWebhookRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Empty])
				}
				object processGitHubEnterpriseWebhook {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): processGitHubEnterpriseWebhook = new processGitHubEnterpriseWebhook(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/connections:processGitHubEnterpriseWebhook")).addQueryStringParameters("parent" -> parent.toString))
				}
				class fetchGitHubInstallations(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.FetchGitHubInstallationsResponse]) {
					def apply() = req.execute("GET").map(_.json.as[Schema.FetchGitHubInstallationsResponse])
				}
				object fetchGitHubInstallations {
					def apply(projectsId :PlayApi, locationsId :PlayApi, connectionsId :PlayApi, connection: String)(using auth: AuthToken, ec: ExecutionContext): fetchGitHubInstallations = new fetchGitHubInstallations(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/connections/${connectionsId}:fetchGitHubInstallations")).addQueryStringParameters("connection" -> connection.toString))
				...
				class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					...
					def withConnection(body: Schema.Connection) = req.withBody(Json.toJson(body)).execute("PATCH").map(_.json.as[Schema.Operation])
				}
				object patch {
				...
				object list {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/connections")).addQueryStringParameters("parent" -> parent.toString))
				...
				object gitRepositoryLinks {
					class processGitLabWebhook(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withProcessGitLabWebhookRequest(body: Schema.ProcessGitLabWebhookRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Empty])
					}
					object processGitLabWebhook {
						def apply(projectsId :PlayApi, locationsId :PlayApi, connectionsId :PlayApi, gitRepositoryLinksId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): processGitLabWebhook = new processGitLabWebhook(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/connections/${connectionsId}/gitRepositoryLinks/${gitRepositoryLinksId}:processGitLabWebhook")).addQueryStringParameters("name" -> name.toString))
					}
					class processGitLabEnterpriseWebhook(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withProcessGitLabEnterpriseWebhookRequest(body: Schema.ProcessGitLabEnterpriseWebhookRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Empty])
					}
					object processGitLabEnterpriseWebhook {
						def apply(projectsId :PlayApi, locationsId :PlayApi, connectionsId :PlayApi, gitRepositoryLinksId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): processGitLabEnterpriseWebhook = new processGitLabEnterpriseWebhook(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/connections/${connectionsId}/gitRepositoryLinks/${gitRepositoryLinksId}:processGitLabEnterpriseWebhook")).addQueryStringParameters("name" -> name.toString))
					}
					class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						/** Optional. An optional request ID to identify requests. Specify a unique request ID so that if you must retry your request, the server will know to ignore the request if it has already been completed. The server will guarantee that for at least 60 minutes since the first request. For example, consider a situation where you make an initial request and the request times out. If you make the request again with the same request ID, the server can check if original operation with the same request ID was received, and if so, will ignore the second request. This prevents clients from accidentally creating duplicate commitments. The request ID must be a valid UUID with the exception that zero UUID is not supported (00000000-0000-0000-0000-000000000000). */
						def withRequestId(requestId: String) = new create(req.addQueryStringParameters("requestId" -> requestId.toString))
						/** Optional. If set, validate the request, but do not actually post it. */
						def withValidateOnly(validateOnly: Boolean) = new create(req.addQueryStringParameters("validateOnly" -> validateOnly.toString))
						def withGitRepositoryLink(body: Schema.GitRepositoryLink) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Operation])
					}
					object create {
					...
					object delete {
						def apply(projectsId :PlayApi, locationsId :PlayApi, connectionsId :PlayApi, gitRepositoryLinksId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/connections/${connectionsId}/gitRepositoryLinks/${gitRepositoryLinksId}")).addQueryStringParameters("name" -> name.toString))
						given Conversion[delete, Future[Schema.Operation]] = (fun: delete) => fun.apply()
					}
					class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GitRepositoryLink]) {
						def apply() = req.execute("GET").map(_.json.as[Schema.GitRepositoryLink])
					}
					object get {
						def apply(projectsId :PlayApi, locationsId :PlayApi, connectionsId :PlayApi, gitRepositoryLinksId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/connections/${connectionsId}/gitRepositoryLinks/${gitRepositoryLinksId}")).addQueryStringParameters("name" -> name.toString))
					...
					class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListGitRepositoryLinksResponse]) {
						...
						/** Optional. Hint for how to order the results */
						def withOrderBy(orderBy: String) = new list(req.addQueryStringParameters("orderBy" -> orderBy.toString))
						def apply() = req.execute("GET").map(_.json.as[Schema.ListGitRepositoryLinksResponse])
					}
					object list {
						def apply(projectsId :PlayApi, locationsId :PlayApi, connectionsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/connections/${connectionsId}/gitRepositoryLinks")).addQueryStringParameters("parent" -> parent.toString))
