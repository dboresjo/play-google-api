		...
		object locations {
			...
			object deployments {
				...
				class deleteState(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withDeleteStatefileRequest(body: Schema.DeleteStatefileRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Empty])
				}
				object deleteState {
					def apply(projectsId :PlayApi, locationsId :PlayApi, deploymentsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): deleteState = new deleteState(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/deployments/${deploymentsId}:deleteState").addQueryStringParameters("name" -> name.toString))
				}
				class exportState(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withExportDeploymentStatefileRequest(body: Schema.ExportDeploymentStatefileRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Statefile])
				}
				object exportState {
				...
				class lock(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withLockDeploymentRequest(body: Schema.LockDeploymentRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object lock {
					def apply(projectsId :PlayApi, locationsId :PlayApi, deploymentsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): lock = new lock(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/deployments/${deploymentsId}:lock").addQueryStringParameters("name" -> name.toString))
				}
				class unlock(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withUnlockDeploymentRequest(body: Schema.UnlockDeploymentRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object unlock {
					def apply(projectsId :PlayApi, locationsId :PlayApi, deploymentsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): unlock = new unlock(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/deployments/${deploymentsId}:unlock").addQueryStringParameters("name" -> name.toString))
				}
				class exportLock(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.LockInfo]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.LockInfo])
				}
				object exportLock {
					def apply(projectsId :PlayApi, locationsId :PlayApi, deploymentsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): exportLock = new exportLock(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/deployments/${deploymentsId}:exportLock").addQueryStringParameters("name" -> name.toString))
				...
				object delete {
					def apply(projectsId :PlayApi, locationsId :PlayApi, deploymentsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/deployments/${deploymentsId}").addQueryStringParameters("name" -> name.toString))
				...
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, deploymentsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/deployments/${deploymentsId}").addQueryStringParameters("name" -> name.toString))
				...
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					...
					def withDeployment(body: Schema.Deployment) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object create {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String, deploymentId: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/deployments").addQueryStringParameters("parent" -> parent.toString, "deploymentId" -> deploymentId.toString))
				}
				...
				class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					...
					def withDeployment(body: Schema.Deployment) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.Operation])
				}
				object patch {
					def apply(projectsId :PlayApi, locationsId :PlayApi, deploymentsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): patch = new patch(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/deployments/${deploymentsId}").addQueryStringParameters("name" -> name.toString))
				}
				class importState(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withImportStatefileRequest(body: Schema.ImportStatefileRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Statefile])
				}
				object importState {
				...
				object revisions {
					class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListRevisionsResponse]) {
						def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListRevisionsResponse])
					}
					object list {
					...
					class exportState(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withExportRevisionStatefileRequest(body: Schema.ExportRevisionStatefileRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Statefile])
					}
					object exportState {
					...
					object resources {
						class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Resource]) {
							def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.Resource])
						}
						object get {
						...
						class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListResourcesResponse]) {
							def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListResourcesResponse])
						}
						object list {
			...
			object terraformVersions {
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListTerraformVersionsResponse]) {
					...
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListTerraformVersionsResponse])
				}
				object list {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/terraformVersions").addQueryStringParameters("parent" -> parent.toString))
				...
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.TerraformVersion]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.TerraformVersion])
				}
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, terraformVersionsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/terraformVersions/${terraformVersionsId}").addQueryStringParameters("name" -> name.toString))
			...
			object previews {
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					...
					/** Optional. An optional request ID to identify requests. Specify a unique request ID so that if you must retry your request, the server will know to ignore the request if it has already been completed. The server will guarantee that for at least 60 minutes since the first request. For example, consider a situation where you make an initial request and the request times out. If you make the request again with the same request ID, the server can check if original operation with the same request ID was received, and if so, will ignore the second request. This prevents clients from accidentally creating duplicate commitments. The request ID must be a valid UUID with the exception that zero UUID is not supported (00000000-0000-0000-0000-000000000000). */
					def withRequestId(requestId: String) = new create(req.addQueryStringParameters("requestId" -> requestId.toString))
					def withPreview(body: Schema.Preview) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object create {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/previews").addQueryStringParameters("parent" -> parent.toString))
				}
				class `export`(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withExportPreviewResultRequest(body: Schema.ExportPreviewResultRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.ExportPreviewResultResponse])
				}
				object `export` {
					def apply(projectsId :PlayApi, locationsId :PlayApi, previewsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): `export` = new `export`(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/previews/${previewsId}:export").addQueryStringParameters("parent" -> parent.toString))
				}
				class delete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Operation]) {
					/** Optional. An optional request ID to identify requests. Specify a unique request ID so that if you must retry your request, the server will know to ignore the request if it has already been completed. The server will guarantee that for at least 60 minutes after the first request. For example, consider a situation where you make an initial request and the request times out. If you make the request again with the same request ID, the server can check if original operation with the same request ID was received, and if so, will ignore the second request. This prevents clients from accidentally creating duplicate commitments. The request ID must be a valid UUID with the exception that zero UUID is not supported (00000000-0000-0000-0000-000000000000). */
					def withRequestId(requestId: String) = new delete(req.addQueryStringParameters("requestId" -> requestId.toString))
				...
				object delete {
					def apply(projectsId :PlayApi, locationsId :PlayApi, previewsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/previews/${previewsId}").addQueryStringParameters("name" -> name.toString))
					given Conversion[delete, Future[Schema.Operation]] = (fun: delete) => fun.apply()
				}
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Preview]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.Preview])
				}
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, previewsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/previews/${previewsId}").addQueryStringParameters("name" -> name.toString))
				...
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListPreviewsResponse]) {
					...
					/** Optional. Field to use to sort the list. */
					def withOrderBy(orderBy: String) = new list(req.addQueryStringParameters("orderBy" -> orderBy.toString))
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListPreviewsResponse])
				}
				object list {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/previews").addQueryStringParameters("parent" -> parent.toString))
