		...
		object locations {
			object pipelines {
				class run(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withGoogleCloudDatapipelinesV1RunPipelineRequest(body: Schema.GoogleCloudDatapipelinesV1RunPipelineRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.GoogleCloudDatapipelinesV1RunPipelineResponse])
				}
				object run {
					def apply(projectsId :PlayApi, locationsId :PlayApi, pipelinesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): run = new run(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/pipelines/${pipelinesId}:run").addQueryStringParameters("name" -> name.toString))
				}
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withGoogleCloudDatapipelinesV1Pipeline(body: Schema.GoogleCloudDatapipelinesV1Pipeline) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.GoogleCloudDatapipelinesV1Pipeline])
				}
				object create {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/pipelines").addQueryStringParameters("parent" -> parent.toString))
				}
				class stop(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withGoogleCloudDatapipelinesV1StopPipelineRequest(body: Schema.GoogleCloudDatapipelinesV1StopPipelineRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.GoogleCloudDatapipelinesV1Pipeline])
				}
				object stop {
					def apply(projectsId :PlayApi, locationsId :PlayApi, pipelinesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): stop = new stop(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/pipelines/${pipelinesId}:stop").addQueryStringParameters("name" -> name.toString))
				}
				...
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleCloudDatapipelinesV1Pipeline]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.GoogleCloudDatapipelinesV1Pipeline])
				}
				object get {
				...
				class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withGoogleCloudDatapipelinesV1Pipeline(body: Schema.GoogleCloudDatapipelinesV1Pipeline) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.GoogleCloudDatapipelinesV1Pipeline])
				}
				object patch {
				...
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleCloudDatapipelinesV1ListPipelinesResponse]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.GoogleCloudDatapipelinesV1ListPipelinesResponse])
				}
				object list {
				...
				object jobs {
					class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleCloudDatapipelinesV1ListJobsResponse]) {
						def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.GoogleCloudDatapipelinesV1ListJobsResponse])
					}
					object list {
