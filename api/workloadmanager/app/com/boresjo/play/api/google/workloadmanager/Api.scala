		...
		object locations {
			...
			object insights {
				class writeInsight(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withWriteInsightRequest(body: Schema.WriteInsightRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.WriteInsightResponse])
				}
				object writeInsight {
					def apply(projectsId :PlayApi, locationsId :PlayApi, location: String)(using auth: AuthToken, ec: ExecutionContext): writeInsight = new writeInsight(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/insights:writeInsight")).addQueryStringParameters("location" -> location.toString))
				}
			}
			object evaluations {
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					...
					def withEvaluation(body: Schema.Evaluation) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Operation])
				}
				object create {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String, evaluationId: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/evaluations")).addQueryStringParameters("parent" -> parent.toString, "evaluationId" -> evaluationId.toString))
				}
				...
				object delete {
					def apply(projectsId :PlayApi, locationsId :PlayApi, evaluationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/evaluations/${evaluationsId}")).addQueryStringParameters("name" -> name.toString))
				...
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Evaluation]) {
					def apply() = req.execute("GET").map(_.json.as[Schema.Evaluation])
				}
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, evaluationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/evaluations/${evaluationsId}")).addQueryStringParameters("name" -> name.toString))
				...
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListEvaluationsResponse]) {
					def apply() = req.execute("GET").map(_.json.as[Schema.ListEvaluationsResponse])
				}
				object list {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String, pageSize: Int, pageToken: String, filter: String, orderBy: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/evaluations")).addQueryStringParameters("parent" -> parent.toString, "pageSize" -> pageSize.toString, "pageToken" -> pageToken.toString, "filter" -> filter.toString, "orderBy" -> orderBy.toString))
				...
				object executions {
					class run(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withRunEvaluationRequest(body: Schema.RunEvaluationRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Operation])
					}
					object run {
						def apply(projectsId :PlayApi, locationsId :PlayApi, evaluationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): run = new run(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/evaluations/${evaluationsId}/executions:run")).addQueryStringParameters("name" -> name.toString))
					}
					class delete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Operation]) {
						/** Optional. An optional request ID to identify requests. Specify a unique request ID so that if you must retry your request, the server will know to ignore the request if it has already been completed. The server will guarantee that for at least 60 minutes after the first request. For example, consider a situation where you make an initial request and the request times out. If you make the request again with the same request ID, the server can check if original operation with the same request ID was received, and if so, will ignore the second request. This prevents clients from accidentally creating duplicate commitments. The request ID must be a valid UUID with the exception that zero UUID is not supported (00000000-0000-0000-0000-000000000000). */
						def withRequestId(requestId: String) = new delete(req.addQueryStringParameters("requestId" -> requestId.toString))
					...
					object delete {
						def apply(projectsId :PlayApi, locationsId :PlayApi, evaluationsId :PlayApi, executionsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/evaluations/${evaluationsId}/executions/${executionsId}")).addQueryStringParameters("name" -> name.toString))
						given Conversion[delete, Future[Schema.Operation]] = (fun: delete) => fun.apply()
					}
					...
					object get {
						def apply(projectsId :PlayApi, locationsId :PlayApi, evaluationsId :PlayApi, executionsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/evaluations/${evaluationsId}/executions/${executionsId}")).addQueryStringParameters("name" -> name.toString))
					...
					object list {
						def apply(projectsId :PlayApi, locationsId :PlayApi, evaluationsId :PlayApi, parent: String, pageSize: Int, pageToken: String, filter: String, orderBy: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/evaluations/${evaluationsId}/executions")).addQueryStringParameters("parent" -> parent.toString, "pageSize" -> pageSize.toString, "pageToken" -> pageToken.toString, "filter" -> filter.toString, "orderBy" -> orderBy.toString))
					...
					object results {
						class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListExecutionResultsResponse]) {
							def apply() = req.execute("GET").map(_.json.as[Schema.ListExecutionResultsResponse])
						}
						object list {
							def apply(projectsId :PlayApi, locationsId :PlayApi, evaluationsId :PlayApi, executionsId :PlayApi, parent: String, pageSize: Int, pageToken: String, filter: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/evaluations/${evaluationsId}/executions/${executionsId}/results")).addQueryStringParameters("parent" -> parent.toString, "pageSize" -> pageSize.toString, "pageToken" -> pageToken.toString, "filter" -> filter.toString))
