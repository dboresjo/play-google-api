...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object folders {
		class updateAutokeyConfig(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withAutokeyConfig(body: Schema.AutokeyConfig) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.AutokeyConfig])
		}
		object updateAutokeyConfig {
			def apply(foldersId :PlayApi, name: String, updateMask: String)(using auth: AuthToken, ec: ExecutionContext): updateAutokeyConfig = new updateAutokeyConfig(ws.url(BASE_URL + s"v1/folders/${foldersId}/autokeyConfig").addQueryStringParameters("name" -> name.toString, "updateMask" -> updateMask.toString))
		}
		class getAutokeyConfig(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.AutokeyConfig]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.AutokeyConfig])
		}
		object getAutokeyConfig {
			def apply(foldersId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): getAutokeyConfig = new getAutokeyConfig(ws.url(BASE_URL + s"v1/folders/${foldersId}/autokeyConfig").addQueryStringParameters("name" -> name.toString))
	...
	object projects {
		class showEffectiveAutokeyConfig(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ShowEffectiveAutokeyConfigResponse]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ShowEffectiveAutokeyConfigResponse])
		}
		object showEffectiveAutokeyConfig {
			def apply(projectsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): showEffectiveAutokeyConfig = new showEffectiveAutokeyConfig(ws.url(BASE_URL + s"v1/projects/${projectsId}:showEffectiveAutokeyConfig").addQueryStringParameters("parent" -> parent.toString))
		...
		object locations {
			class updateEkmConfig(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				def withEkmConfig(body: Schema.EkmConfig) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.EkmConfig])
			}
			...
			class generateRandomBytes(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				def withGenerateRandomBytesRequest(body: Schema.GenerateRandomBytesRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.GenerateRandomBytesResponse])
			}
			object generateRandomBytes {
				def apply(projectsId :PlayApi, locationsId :PlayApi, location: String)(using auth: AuthToken, ec: ExecutionContext): generateRandomBytes = new generateRandomBytes(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}:generateRandomBytes").addQueryStringParameters("location" -> location.toString))
			}
			class getEkmConfig(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.EkmConfig]) {
				def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.EkmConfig])
			}
			object getEkmConfig {
				def apply(projectsId :PlayApi, locationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): getEkmConfig = new getEkmConfig(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/ekmConfig").addQueryStringParameters("name" -> name.toString))
			...
			object keyRings {
				...
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.KeyRing]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.KeyRing])
				}
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, keyRingsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/keyRings/${keyRingsId}").addQueryStringParameters("name" -> name.toString))
				...
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListKeyRingsResponse]) {
					...
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListKeyRingsResponse])
				}
				object list {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/keyRings").addQueryStringParameters("parent" -> parent.toString))
					given Conversion[list, Future[Schema.ListKeyRingsResponse]] = (fun: list) => fun.apply()
				}
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withKeyRing(body: Schema.KeyRing) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.KeyRing])
				}
				object create {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String, keyRingId: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/keyRings").addQueryStringParameters("parent" -> parent.toString, "keyRingId" -> keyRingId.toString))
				}
				...
				object cryptoKeys {
					...
					class encrypt(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withEncryptRequest(body: Schema.EncryptRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.EncryptResponse])
					}
					object encrypt {
					...
					class decrypt(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withDecryptRequest(body: Schema.DecryptRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.DecryptResponse])
					}
					object decrypt {
					...
					class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.CryptoKey]) {
						def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.CryptoKey])
					}
					object get {
					...
					class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withCryptoKey(body: Schema.CryptoKey) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.CryptoKey])
					}
					object patch {
					...
					class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListCryptoKeysResponse]) {
						...
						/** Optional. Specify how the results should be sorted. If not specified, the results will be sorted in the default order. For more information, see [Sorting and filtering list results](https://cloud.google.com/kms/docs/sorting-and-filtering). */
						def withOrderBy(orderBy: String) = new list(req.addQueryStringParameters("orderBy" -> orderBy.toString))
						def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListCryptoKeysResponse])
					}
					object list {
					...
					class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withCryptoKey(body: Schema.CryptoKey) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.CryptoKey])
					}
					object create {
					...
					class updatePrimaryVersion(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withUpdateCryptoKeyPrimaryVersionRequest(body: Schema.UpdateCryptoKeyPrimaryVersionRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.CryptoKey])
					}
					...
					object cryptoKeyVersions {
						class getPublicKey(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.PublicKey]) {
							def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.PublicKey])
						}
						object getPublicKey {
						...
						class asymmetricSign(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
							def withAsymmetricSignRequest(body: Schema.AsymmetricSignRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.AsymmetricSignResponse])
						}
						...
						class restore(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
							def withRestoreCryptoKeyVersionRequest(body: Schema.RestoreCryptoKeyVersionRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.CryptoKeyVersion])
						}
						object restore {
						...
						class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
							def withCryptoKeyVersion(body: Schema.CryptoKeyVersion) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.CryptoKeyVersion])
						}
						object create {
						...
						class rawDecrypt(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
							def withRawDecryptRequest(body: Schema.RawDecryptRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.RawDecryptResponse])
						}
						object rawDecrypt {
						...
						class macVerify(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
							def withMacVerifyRequest(body: Schema.MacVerifyRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.MacVerifyResponse])
						}
						object macVerify {
						...
						class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.CryptoKeyVersion]) {
							def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.CryptoKeyVersion])
						}
						object get {
						...
						class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
							def withCryptoKeyVersion(body: Schema.CryptoKeyVersion) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.CryptoKeyVersion])
						}
						object patch {
						...
						class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListCryptoKeyVersionsResponse]) {
							...
							/** Optional. Specify how the results should be sorted. If not specified, the results will be sorted in the default order. For more information, see [Sorting and filtering list results](https://cloud.google.com/kms/docs/sorting-and-filtering). */
							def withOrderBy(orderBy: String) = new list(req.addQueryStringParameters("orderBy" -> orderBy.toString))
							def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListCryptoKeyVersionsResponse])
						}
						object list {
						...
						class `import`(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
							def withImportCryptoKeyVersionRequest(body: Schema.ImportCryptoKeyVersionRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.CryptoKeyVersion])
						}
						object `import` {
						...
						class destroy(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
							def withDestroyCryptoKeyVersionRequest(body: Schema.DestroyCryptoKeyVersionRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.CryptoKeyVersion])
						}
						object destroy {
						...
						class macSign(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
							def withMacSignRequest(body: Schema.MacSignRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.MacSignResponse])
						}
						object macSign {
						...
						class asymmetricDecrypt(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
							def withAsymmetricDecryptRequest(body: Schema.AsymmetricDecryptRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.AsymmetricDecryptResponse])
						}
						...
						class rawEncrypt(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
							def withRawEncryptRequest(body: Schema.RawEncryptRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.RawEncryptResponse])
						}
						object rawEncrypt {
				...
				object importJobs {
					...
					object list {
						def apply(projectsId :PlayApi, locationsId :PlayApi, keyRingsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/keyRings/${keyRingsId}/importJobs").addQueryStringParameters("parent" -> parent.toString))
						given Conversion[list, Future[Schema.ListImportJobsResponse]] = (fun: list) => fun.apply()
					}
					class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withImportJob(body: Schema.ImportJob) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.ImportJob])
					}
					object create {
			...
			object ekmConnections {
				...
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.EkmConnection]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.EkmConnection])
				}
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, ekmConnectionsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/ekmConnections/${ekmConnectionsId}").addQueryStringParameters("name" -> name.toString))
					given Conversion[get, Future[Schema.EkmConnection]] = (fun: get) => fun.apply()
				}
				class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withEkmConnection(body: Schema.EkmConnection) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.EkmConnection])
				}
				object patch {
				...
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withEkmConnection(body: Schema.EkmConnection) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.EkmConnection])
				}
				object create {
				...
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListEkmConnectionsResponse]) {
					...
					/** Optional. Specify how the results should be sorted. If not specified, the results will be sorted in the default order. For more information, see [Sorting and filtering list results](https://cloud.google.com/kms/docs/sorting-and-filtering). */
					def withOrderBy(orderBy: String) = new list(req.addQueryStringParameters("orderBy" -> orderBy.toString))
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListEkmConnectionsResponse])
				}
				object list {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/ekmConnections").addQueryStringParameters("parent" -> parent.toString))
				...
				class verifyConnectivity(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.VerifyConnectivityResponse]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.VerifyConnectivityResponse])
				}
			...
			object ekmConfig {
				...
				object setIamPolicy {
					def apply(projectsId :PlayApi, locationsId :PlayApi, resource: String)(using auth: AuthToken, ec: ExecutionContext): setIamPolicy = new setIamPolicy(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/ekmConfig:setIamPolicy").addQueryStringParameters("resource" -> resource.toString))
				}
				class getIamPolicy(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Policy]) {
					/** Optional. The maximum policy version that will be used to format the policy. Valid values are 0, 1, and 3. Requests specifying an invalid value will be rejected. Requests for policies with any conditional role bindings must specify version 3. Policies with no conditional role bindings may specify any valid value or leave the field unset. The policy in the response might use the policy version that you specified, or it might use a lower policy version. For example, if you specify version 3, but the policy has no conditional role bindings, the response uses version 1. To learn which resources support conditions in their IAM policies, see the [IAM documentation](https://cloud.google.com/iam/help/conditions/resource-policies).<br>Format: int32 */
					def withOptionsRequestedPolicyVersion(optionsRequestedPolicyVersion: Int) = new getIamPolicy(req.addQueryStringParameters("options.requestedPolicyVersion" -> optionsRequestedPolicyVersion.toString))
				...
				object getIamPolicy {
					def apply(projectsId :PlayApi, locationsId :PlayApi, resource: String)(using auth: AuthToken, ec: ExecutionContext): getIamPolicy = new getIamPolicy(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/ekmConfig:getIamPolicy").addQueryStringParameters("resource" -> resource.toString))
					given Conversion[getIamPolicy, Future[Schema.Policy]] = (fun: getIamPolicy) => fun.apply()
				}
				class testIamPermissions(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withTestIamPermissionsRequest(body: Schema.TestIamPermissionsRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.TestIamPermissionsResponse])
				...
				object testIamPermissions {
					def apply(projectsId :PlayApi, locationsId :PlayApi, resource: String)(using auth: AuthToken, ec: ExecutionContext): testIamPermissions = new testIamPermissions(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/ekmConfig:testIamPermissions").addQueryStringParameters("resource" -> resource.toString))
				}
			}
			object keyHandles {
				class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					...
					def withKeyHandle(body: Schema.KeyHandle) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object create {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/keyHandles").addQueryStringParameters("parent" -> parent.toString))
				}
				class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.KeyHandle]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.KeyHandle])
				}
				object get {
					def apply(projectsId :PlayApi, locationsId :PlayApi, keyHandlesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/keyHandles/${keyHandlesId}").addQueryStringParameters("name" -> name.toString))
				...
				class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListKeyHandlesResponse]) {
					...
					def withFilter(filter: String) = new list(req.addQueryStringParameters("filter" -> filter.toString))
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListKeyHandlesResponse])
				}
				object list {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v1/projects/${projectsId}/locations/${locationsId}/keyHandles").addQueryStringParameters("parent" -> parent.toString))
