...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object operations {
		...
		object list {
			def apply(name: String, filter: String, pageSize: Int, pageToken: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"operations")).addQueryStringParameters("name" -> name.toString, "filter" -> filter.toString, "pageSize" -> pageSize.toString, "pageToken" -> pageToken.toString))
		...
		object get {
			def apply(name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"operations/${name}")).addQueryStringParameters())
	...
	object operation {
		...
		object delete {
			def apply(name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"operation/${name}")).addQueryStringParameters())
		...
		object cancel {
			def apply(name: String)(using auth: AuthToken, ec: ExecutionContext): cancel = new cancel(auth(ws.url(BASE_URL + s"operation/${name}:cancel")).addQueryStringParameters())
	...
	object about {
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.About]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.About])
		}
		object get {
			def apply()(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"about")).addQueryStringParameters())
	...
	object apps {
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.App]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.App])
		}
		object get {
			def apply(appId: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"apps/${appId}")).addQueryStringParameters())
		...
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.AppList]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.AppList])
		}
		object list {
			def apply(appFilterExtensions: String, appFilterMimeTypes: String, languageCode: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"apps")).addQueryStringParameters("appFilterExtensions" -> appFilterExtensions.toString, "appFilterMimeTypes" -> appFilterMimeTypes.toString, "languageCode" -> languageCode.toString))
	...
	object comments {
		...
		object create {
			def apply(fileId: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(auth(ws.url(BASE_URL + s"files/${fileId}/comments")).addQueryStringParameters())
		}
		class delete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Unit]) {
			def apply() = req.execute("DELETE").map(_ => ())
		}
		object delete {
			def apply(fileId: String, commentId: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"files/${fileId}/comments/${commentId}")).addQueryStringParameters())
			given Conversion[delete, Future[Unit]] = (fun: delete) => fun.apply()
		}
		...
		object get {
			def apply(fileId: String, commentId: String, includeDeleted: Boolean)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"files/${fileId}/comments/${commentId}")).addQueryStringParameters("includeDeleted" -> includeDeleted.toString))
			given Conversion[get, Future[Schema.Comment]] = (fun: get) => fun.apply()
		}
		class update(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withComment(body: Schema.Comment) = req.withBody(Json.toJson(body)).execute("PATCH").map(_.json.as[Schema.Comment])
		}
		object update {
			def apply(fileId: String, commentId: String)(using auth: AuthToken, ec: ExecutionContext): update = new update(auth(ws.url(BASE_URL + s"files/${fileId}/comments/${commentId}")).addQueryStringParameters())
		}
		...
		object list {
			def apply(fileId: String, includeDeleted: Boolean, pageSize: Int, pageToken: String, startModifiedTime: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"files/${fileId}/comments")).addQueryStringParameters("includeDeleted" -> includeDeleted.toString, "pageSize" -> pageSize.toString, "pageToken" -> pageToken.toString, "startModifiedTime" -> startModifiedTime.toString))
			given Conversion[list, Future[Schema.CommentList]] = (fun: list) => fun.apply()
		}
	}
	object drives {
		class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withDrive(body: Schema.Drive) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Drive])
		}
		object create {
			def apply(requestId: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(auth(ws.url(BASE_URL + s"drives")).addQueryStringParameters("requestId" -> requestId.toString))
		}
		class hide(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Drive]) {
			def apply() = req.execute("POST").map(_.json.as[Schema.Drive])
		}
		object hide {
			def apply(driveId: String)(using auth: AuthToken, ec: ExecutionContext): hide = new hide(auth(ws.url(BASE_URL + s"drives/${driveId}/hide")).addQueryStringParameters())
		...
		object delete {
			def apply(driveId: String, useDomainAdminAccess: Boolean, allowItemDeletion: Boolean)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"drives/${driveId}")).addQueryStringParameters("useDomainAdminAccess" -> useDomainAdminAccess.toString, "allowItemDeletion" -> allowItemDeletion.toString))
			given Conversion[delete, Future[Unit]] = (fun: delete) => fun.apply()
		}
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Drive]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.Drive])
		}
		object get {
			def apply(driveId: String, useDomainAdminAccess: Boolean)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"drives/${driveId}")).addQueryStringParameters("useDomainAdminAccess" -> useDomainAdminAccess.toString))
			given Conversion[get, Future[Schema.Drive]] = (fun: get) => fun.apply()
		}
		class update(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withDrive(body: Schema.Drive) = req.withBody(Json.toJson(body)).execute("PATCH").map(_.json.as[Schema.Drive])
		}
		object update {
			def apply(driveId: String, useDomainAdminAccess: Boolean)(using auth: AuthToken, ec: ExecutionContext): update = new update(auth(ws.url(BASE_URL + s"drives/${driveId}")).addQueryStringParameters("useDomainAdminAccess" -> useDomainAdminAccess.toString))
		}
		class unhide(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Drive]) {
			def apply() = req.execute("POST").map(_.json.as[Schema.Drive])
		}
		object unhide {
			def apply(driveId: String)(using auth: AuthToken, ec: ExecutionContext): unhide = new unhide(auth(ws.url(BASE_URL + s"drives/${driveId}/unhide")).addQueryStringParameters())
	...
	object files {
		class generateIds(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GeneratedIds]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.GeneratedIds])
		}
		object generateIds {
			def apply(count: Int, space: String, `type`: String)(using auth: AuthToken, ec: ExecutionContext): generateIds = new generateIds(auth(ws.url(BASE_URL + s"files/generateIds")).addQueryStringParameters("count" -> count.toString, "space" -> space.toString, "type" -> `type`.toString))
		...
		object `export` {
			def apply(fileId: String, mimeType: String)(using auth: AuthToken, ec: ExecutionContext): `export` = new `export`(auth(ws.url(BASE_URL + s"files/${fileId}/export")).addQueryStringParameters("mimeType" -> mimeType.toString))
		...
		object download {
			def apply(fileId: String)(using auth: AuthToken, ec: ExecutionContext): download = new download(auth(ws.url(BASE_URL + s"files/${fileId}/download")).addQueryStringParameters())
		...
		class modifyLabels(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withModifyLabelsRequest(body: Schema.ModifyLabelsRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.ModifyLabelsResponse])
		}
		object modifyLabels {
			def apply(fileId: String)(using auth: AuthToken, ec: ExecutionContext): modifyLabels = new modifyLabels(auth(ws.url(BASE_URL + s"files/${fileId}/modifyLabels")).addQueryStringParameters())
		}
		class listLabels(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.LabelList]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.LabelList])
		}
		object listLabels {
			def apply(fileId: String, maxResults: Int, pageToken: String)(using auth: AuthToken, ec: ExecutionContext): listLabels = new listLabels(auth(ws.url(BASE_URL + s"files/${fileId}/listLabels")).addQueryStringParameters("maxResults" -> maxResults.toString, "pageToken" -> pageToken.toString))
	...
	object permissions {
		...
		object get {
			def apply(fileId: String, permissionId: String, supportsAllDrives: Boolean, supportsTeamDrives: Boolean, useDomainAdminAccess: Boolean)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"files/${fileId}/permissions/${permissionId}")).addQueryStringParameters("supportsAllDrives" -> supportsAllDrives.toString, "supportsTeamDrives" -> supportsTeamDrives.toString, "useDomainAdminAccess" -> useDomainAdminAccess.toString))
			given Conversion[get, Future[Schema.Permission]] = (fun: get) => fun.apply()
		}
		class update(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withPermission(body: Schema.Permission) = req.withBody(Json.toJson(body)).execute("PATCH").map(_.json.as[Schema.Permission])
		}
		object update {
			def apply(fileId: String, permissionId: String, removeExpiration: Boolean, supportsAllDrives: Boolean, supportsTeamDrives: Boolean, transferOwnership: Boolean, useDomainAdminAccess: Boolean)(using auth: AuthToken, ec: ExecutionContext): update = new update(auth(ws.url(BASE_URL + s"files/${fileId}/permissions/${permissionId}")).addQueryStringParameters("removeExpiration" -> removeExpiration.toString, "supportsAllDrives" -> supportsAllDrives.toString, "supportsTeamDrives" -> supportsTeamDrives.toString, "transferOwnership" -> transferOwnership.toString, "useDomainAdminAccess" -> useDomainAdminAccess.toString))
		}
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.PermissionList]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.PermissionList])
		}
		object list {
			def apply(fileId: String, pageSize: Int, pageToken: String, supportsAllDrives: Boolean, supportsTeamDrives: Boolean, useDomainAdminAccess: Boolean, includePermissionsForView: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"files/${fileId}/permissions")).addQueryStringParameters("pageSize" -> pageSize.toString, "pageToken" -> pageToken.toString, "supportsAllDrives" -> supportsAllDrives.toString, "supportsTeamDrives" -> supportsTeamDrives.toString, "useDomainAdminAccess" -> useDomainAdminAccess.toString, "includePermissionsForView" -> includePermissionsForView.toString))
			given Conversion[list, Future[Schema.PermissionList]] = (fun: list) => fun.apply()
		}
	}
	object replies {
		class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withReply(body: Schema.Reply) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.Reply])
		}
		object create {
			def apply(fileId: String, commentId: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(auth(ws.url(BASE_URL + s"files/${fileId}/comments/${commentId}/replies")).addQueryStringParameters())
		}
		class delete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Unit]) {
			def apply() = req.execute("DELETE").map(_ => ())
		}
		object delete {
			def apply(fileId: String, commentId: String, replyId: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"files/${fileId}/comments/${commentId}/replies/${replyId}")).addQueryStringParameters())
			given Conversion[delete, Future[Unit]] = (fun: delete) => fun.apply()
		}
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Reply]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.Reply])
		}
		object get {
			def apply(fileId: String, commentId: String, replyId: String, includeDeleted: Boolean)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"files/${fileId}/comments/${commentId}/replies/${replyId}")).addQueryStringParameters("includeDeleted" -> includeDeleted.toString))
			given Conversion[get, Future[Schema.Reply]] = (fun: get) => fun.apply()
		}
		class update(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withReply(body: Schema.Reply) = req.withBody(Json.toJson(body)).execute("PATCH").map(_.json.as[Schema.Reply])
		}
		object update {
			def apply(fileId: String, commentId: String, replyId: String)(using auth: AuthToken, ec: ExecutionContext): update = new update(auth(ws.url(BASE_URL + s"files/${fileId}/comments/${commentId}/replies/${replyId}")).addQueryStringParameters())
		}
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ReplyList]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.ReplyList])
		}
		object list {
			def apply(fileId: String, commentId: String, includeDeleted: Boolean, pageSize: Int, pageToken: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"files/${fileId}/comments/${commentId}/replies")).addQueryStringParameters("includeDeleted" -> includeDeleted.toString, "pageSize" -> pageSize.toString, "pageToken" -> pageToken.toString))
	...
	object revisions {
		...
		object delete {
			def apply(fileId: String, revisionId: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"files/${fileId}/revisions/${revisionId}")).addQueryStringParameters())
			given Conversion[delete, Future[Unit]] = (fun: delete) => fun.apply()
		}
		...
		object get {
			def apply(fileId: String, revisionId: String, acknowledgeAbuse: Boolean)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"files/${fileId}/revisions/${revisionId}")).addQueryStringParameters("acknowledgeAbuse" -> acknowledgeAbuse.toString))
		...
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.RevisionList]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.RevisionList])
		}
		object list {
			def apply(fileId: String, pageSize: Int, pageToken: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"files/${fileId}/revisions")).addQueryStringParameters("pageSize" -> pageSize.toString, "pageToken" -> pageToken.toString))
			given Conversion[list, Future[Schema.RevisionList]] = (fun: list) => fun.apply()
		}
		class update(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withRevision(body: Schema.Revision) = req.withBody(Json.toJson(body)).execute("PATCH").map(_.json.as[Schema.Revision])
		}
		object update {
			def apply(fileId: String, revisionId: String)(using auth: AuthToken, ec: ExecutionContext): update = new update(auth(ws.url(BASE_URL + s"files/${fileId}/revisions/${revisionId}")).addQueryStringParameters())
		}
	}
	object teamdrives {
		class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withTeamDrive(body: Schema.TeamDrive) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.TeamDrive])
		}
		object create {
			def apply(requestId: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(auth(ws.url(BASE_URL + s"teamdrives")).addQueryStringParameters("requestId" -> requestId.toString))
		}
		class delete(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Unit]) {
			def apply() = req.execute("DELETE").map(_ => ())
		}
		object delete {
			def apply(teamDriveId: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"teamdrives/${teamDriveId}")).addQueryStringParameters())
			given Conversion[delete, Future[Unit]] = (fun: delete) => fun.apply()
		}
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.TeamDrive]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.TeamDrive])
		}
		object get {
			def apply(teamDriveId: String, useDomainAdminAccess: Boolean)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"teamdrives/${teamDriveId}")).addQueryStringParameters("useDomainAdminAccess" -> useDomainAdminAccess.toString))
			given Conversion[get, Future[Schema.TeamDrive]] = (fun: get) => fun.apply()
		}
		class update(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withTeamDrive(body: Schema.TeamDrive) = req.withBody(Json.toJson(body)).execute("PATCH").map(_.json.as[Schema.TeamDrive])
		}
		object update {
			def apply(teamDriveId: String, useDomainAdminAccess: Boolean)(using auth: AuthToken, ec: ExecutionContext): update = new update(auth(ws.url(BASE_URL + s"teamdrives/${teamDriveId}")).addQueryStringParameters("useDomainAdminAccess" -> useDomainAdminAccess.toString))
		}
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.TeamDriveList]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.TeamDriveList])
		}
		object list {
			def apply(pageSize: Int, pageToken: String, q: String, useDomainAdminAccess: Boolean)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"teamdrives")).addQueryStringParameters("pageSize" -> pageSize.toString, "pageToken" -> pageToken.toString, "q" -> q.toString, "useDomainAdminAccess" -> useDomainAdminAccess.toString))
	...
	object accessproposals {
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.AccessProposal]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.AccessProposal])
		}
		object get {
			def apply(fileId: String, proposalId: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"files/${fileId}/accessproposals/${proposalId}")).addQueryStringParameters())
		...
		class resolve(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withResolveAccessProposalRequest(body: Schema.ResolveAccessProposalRequest) = req.withBody(Json.toJson(body)).execute("POST").map(_ => ())
		}
		object resolve {
			def apply(fileId: String, proposalId: String)(using auth: AuthToken, ec: ExecutionContext): resolve = new resolve(auth(ws.url(BASE_URL + s"files/${fileId}/accessproposals/${proposalId}:resolve")).addQueryStringParameters())
		}
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListAccessProposalsResponse]) {
			...
			def apply() = req.execute("GET").map(_.json.as[Schema.ListAccessProposalsResponse])
		}
		object list {
			def apply(fileId: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"files/${fileId}/accessproposals")).addQueryStringParameters())
