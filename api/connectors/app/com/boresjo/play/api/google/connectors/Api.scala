		...
		object locations {
			object connections {
				class exchangeAuthCode(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withExchangeAuthCodeRequest(body: Schema.ExchangeAuthCodeRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.ExchangeAuthCodeResponse])
				}
				...
				class executeSqlQuery(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withExecuteSqlQueryRequest(body: Schema.ExecuteSqlQueryRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.ExecuteSqlQueryResponse])
				}
				...
				class checkStatus(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.CheckStatusResponse]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.CheckStatusResponse])
				}
				object checkStatus {
					def apply(projectsId :PlayApi, locationsId :PlayApi, connectionsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): checkStatus = new checkStatus(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/connections/${connectionsId}:checkStatus").addQueryStringParameters("name" -> name.toString))
				...
				class checkReadiness(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.CheckReadinessResponse]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.CheckReadinessResponse])
				}
				...
				class refreshAccessToken(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withRefreshAccessTokenRequest(body: Schema.RefreshAccessTokenRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.RefreshAccessTokenResponse])
				}
				...
				object actions {
					class execute(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
						def withExecuteActionRequest(body: Schema.ExecuteActionRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.ExecuteActionResponse])
					}
					object execute {
					...
					class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListActionsResponse]) {
						def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListActionsResponse])
					}
					object list {
					...
					class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Action]) {
						def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.Action])
					}
					object get {
				...
				object entityTypes {
					class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.EntityType]) {
						def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.EntityType])
					}
					object get {
					...
					class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListEntityTypesResponse]) {
						def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListEntityTypesResponse])
					}
					object list {
					...
					object entities {
						class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
							def withEntity(body: Schema.Entity) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Entity])
						}
						object create {
						...
						class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Entity]) {
							def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.Entity])
						}
						object get {
							def apply(projectsId :PlayApi, locationsId :PlayApi, connectionsId :PlayApi, entityTypesId :PlayApi, entitiesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/connections/${connectionsId}/entityTypes/${entityTypesId}/entities/${entitiesId}").addQueryStringParameters("name" -> name.toString))
						...
						class updateEntitiesWithConditions(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
							def withEntity(body: Schema.Entity) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.UpdateEntitiesWithConditionsResponse])
						}
						...
						class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
							def withEntity(body: Schema.Entity) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.Entity])
						}
						object patch {
						...
						class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListEntitiesResponse]) {
							def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListEntitiesResponse])
						}
						object list {
