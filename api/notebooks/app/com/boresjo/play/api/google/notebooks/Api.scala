		...
		object locations {
			...
			object instances {
				...
				class resizeDisk(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withResizeDiskRequest(body: Schema.ResizeDiskRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object resizeDisk {
				...
				object restore {
					def apply(projectsId :PlayApi, locationsId :PlayApi, instancesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): restore = new restore(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/instances/${instancesId}:restore").addQueryStringParameters("name" -> name.toString))
				}
				class checkUpgradability(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.CheckInstanceUpgradabilityResponse]) {
					def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.CheckInstanceUpgradabilityResponse])
				}
				...
				class diagnose(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withDiagnoseInstanceRequest(body: Schema.DiagnoseInstanceRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object diagnose {
					def apply(projectsId :PlayApi, locationsId :PlayApi, instancesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): diagnose = new diagnose(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/instances/${instancesId}:diagnose").addQueryStringParameters("name" -> name.toString))
				}
				class rollback(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withRollbackInstanceRequest(body: Schema.RollbackInstanceRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object rollback {
					def apply(projectsId :PlayApi, locationsId :PlayApi, instancesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): rollback = new rollback(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/instances/${instancesId}:rollback").addQueryStringParameters("name" -> name.toString))
				}
				class reportInfoSystem(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withReportInstanceInfoSystemRequest(body: Schema.ReportInstanceInfoSystemRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				...
				object delete {
					def apply(projectsId :PlayApi, locationsId :PlayApi, instancesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/instances/${instancesId}").addQueryStringParameters("name" -> name.toString))
				...
				object getConfig {
					def apply(projectsId :PlayApi, locationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): getConfig = new getConfig(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/instances:getConfig").addQueryStringParameters("name" -> name.toString))
				...
				object list {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/instances").addQueryStringParameters("parent" -> parent.toString))
				...
				object create {
					def apply(projectsId :PlayApi, locationsId :PlayApi, parent: String, instanceId: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/instances").addQueryStringParameters("parent" -> parent.toString, "instanceId" -> instanceId.toString))
				}
				...
				class upgradeSystem(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
					def withUpgradeInstanceSystemRequest(body: Schema.UpgradeInstanceSystemRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Operation])
				}
				object upgradeSystem {
					def apply(projectsId :PlayApi, locationsId :PlayApi, instancesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): upgradeSystem = new upgradeSystem(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/instances/${instancesId}:upgradeSystem").addQueryStringParameters("name" -> name.toString))
				}
				...
				object upgrade {
					def apply(projectsId :PlayApi, locationsId :PlayApi, instancesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): upgrade = new upgrade(ws.url(BASE_URL + s"v2/projects/${projectsId}/locations/${locationsId}/instances/${instancesId}:upgrade").addQueryStringParameters("name" -> name.toString))
				}
