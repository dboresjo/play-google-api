...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object organizations {
		...
		object get {
			def apply(organizationsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v1alpha/organizations/${organizationsId}").addQueryStringParameters("name" -> name.toString))
			given Conversion[get, Future[Schema.Organization]] = (fun: get) => fun.apply()
		}
		object analyticsAccountLinks {
			class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				def withAnalyticsAccountLink(body: Schema.AnalyticsAccountLink) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.AnalyticsAccountLink])
			}
			object create {
				def apply(organizationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): create = new create(ws.url(BASE_URL + s"v1alpha/organizations/${organizationsId}/analyticsAccountLinks").addQueryStringParameters("parent" -> parent.toString))
			}
			class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ListAnalyticsAccountLinksResponse]) {
				...
				def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.ListAnalyticsAccountLinksResponse])
			}
			object list {
				def apply(organizationsId :PlayApi, parent: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(ws.url(BASE_URL + s"v1alpha/organizations/${organizationsId}/analyticsAccountLinks").addQueryStringParameters("parent" -> parent.toString))
			...
			object delete {
				def apply(organizationsId :PlayApi, analyticsAccountLinksId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v1alpha/organizations/${organizationsId}/analyticsAccountLinks/${analyticsAccountLinksId}").addQueryStringParameters("name" -> name.toString))
				given Conversion[delete, Future[Schema.Empty]] = (fun: delete) => fun.apply()
			}
			class setPropertyServiceLevel(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
				def withSetPropertyServiceLevelRequest(body: Schema.SetPropertyServiceLevelRequest) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.SetPropertyServiceLevelResponse])
			}
