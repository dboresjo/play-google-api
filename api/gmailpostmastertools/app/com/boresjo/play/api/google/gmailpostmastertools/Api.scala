...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object domains {
		...
		object list {
			def apply(pageSize: Int, pageToken: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1/domains")).addQueryStringParameters("pageSize" -> pageSize.toString, "pageToken" -> pageToken.toString))
		...
		object get {
			def apply(domainsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v1/domains/${domainsId}")).addQueryStringParameters("name" -> name.toString))
		...
		object trafficStats {
			...
			class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.TrafficStats]) {
				def apply() = req.execute("GET").map(_.json.as[Schema.TrafficStats])
			}
			object get {
				def apply(domainsId :PlayApi, trafficStatsId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v1/domains/${domainsId}/trafficStats/${trafficStatsId}")).addQueryStringParameters("name" -> name.toString))
