...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object postUserInfos {
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.PostUserInfosList]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.PostUserInfosList])
		}
		object list {
		...
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.PostUserInfo]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.PostUserInfo])
		}
		object get {
			def apply(maxComments: Int, postId: String, blogId: String, userId: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v3/users/${userId}/blogs/${blogId}/posts/${postId}").addQueryStringParameters("maxComments" -> maxComments.toString))
	...
	object posts {
		class insert(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withPost(body: Schema.Post) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Post])
		}
		object insert {
			def apply(fetchImages: Boolean, fetchBody: Boolean, isDraft: Boolean, blogId: String)(using auth: AuthToken, ec: ExecutionContext): insert = new insert(ws.url(BASE_URL + s"v3/blogs/${blogId}/posts").addQueryStringParameters("fetchImages" -> fetchImages.toString, "fetchBody" -> fetchBody.toString, "isDraft" -> isDraft.toString))
		}
		class getByPath(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Post]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.Post])
		}
		object getByPath {
			def apply(maxComments: Int, blogId: String, view: String, path: String)(using auth: AuthToken, ec: ExecutionContext): getByPath = new getByPath(ws.url(BASE_URL + s"v3/blogs/${blogId}/posts/bypath").addQueryStringParameters("maxComments" -> maxComments.toString, "view" -> view.toString, "path" -> path.toString))
		...
		class publish(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Post]) {
			def apply() = auth.exec(req,_.execute("POST")).map(_.json.as[Schema.Post])
		}
		object publish {
			def apply(publishDate: String, postId: String, blogId: String)(using auth: AuthToken, ec: ExecutionContext): publish = new publish(ws.url(BASE_URL + s"v3/blogs/${blogId}/posts/${postId}/publish").addQueryStringParameters("publishDate" -> publishDate.toString))
		...
		object delete {
			def apply(useTrash: Boolean, postId: String, blogId: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v3/blogs/${blogId}/posts/${postId}").addQueryStringParameters("useTrash" -> useTrash.toString))
		...
		class update(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withPost(body: Schema.Post) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PUT")).map(_.json.as[Schema.Post])
		}
		object update {
		...
		class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withPost(body: Schema.Post) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.Post])
		}
		object patch {
		...
		object revert {
			def apply(postId: String, blogId: String)(using auth: AuthToken, ec: ExecutionContext): revert = new revert(ws.url(BASE_URL + s"v3/blogs/${blogId}/posts/${postId}/revert").addQueryStringParameters())
		...
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.PostList]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.PostList])
		}
		object list {
		...
		object search {
			def apply(fetchBodies: Boolean, q: String, blogId: String, orderBy: String)(using auth: AuthToken, ec: ExecutionContext): search = new search(ws.url(BASE_URL + s"v3/blogs/${blogId}/posts/search").addQueryStringParameters("fetchBodies" -> fetchBodies.toString, "q" -> q.toString, "orderBy" -> orderBy.toString))
	...
	object blogUserInfos {
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.BlogUserInfo]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.BlogUserInfo])
		}
		object get {
			def apply(userId: String, maxPosts: Int, blogId: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v3/users/${userId}/blogs/${blogId}").addQueryStringParameters("maxPosts" -> maxPosts.toString))
	...
	object blogs {
		class listByUser(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.BlogList]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.BlogList])
		}
		object listByUser {
		...
		class getByUrl(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Blog]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.Blog])
		}
		object getByUrl {
			def apply(view: String, url: String)(using auth: AuthToken, ec: ExecutionContext): getByUrl = new getByUrl(ws.url(BASE_URL + s"v3/blogs/byurl").addQueryStringParameters("view" -> view.toString, "url" -> url.toString))
		...
		object get {
			def apply(view: String, maxPosts: Int, blogId: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v3/blogs/${blogId}").addQueryStringParameters("view" -> view.toString, "maxPosts" -> maxPosts.toString))
	...
	object comments {
		class approve(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Comment]) {
			def apply() = auth.exec(req,_.execute("POST")).map(_.json.as[Schema.Comment])
		}
		object approve {
			def apply(blogId: String, postId: String, commentId: String)(using auth: AuthToken, ec: ExecutionContext): approve = new approve(ws.url(BASE_URL + s"v3/blogs/${blogId}/posts/${postId}/comments/${commentId}/approve").addQueryStringParameters())
		...
		object delete {
			def apply(commentId: String, postId: String, blogId: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v3/blogs/${blogId}/posts/${postId}/comments/${commentId}").addQueryStringParameters())
			given Conversion[delete, Future[Unit]] = (fun: delete) => fun.apply()
		}
		...
		object removeContent {
			def apply(commentId: String, blogId: String, postId: String)(using auth: AuthToken, ec: ExecutionContext): removeContent = new removeContent(ws.url(BASE_URL + s"v3/blogs/${blogId}/posts/${postId}/comments/${commentId}/removecontent").addQueryStringParameters())
		...
		object get {
			def apply(commentId: String, postId: String, blogId: String, view: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v3/blogs/${blogId}/posts/${postId}/comments/${commentId}").addQueryStringParameters("view" -> view.toString))
		...
		object markAsSpam {
			def apply(blogId: String, postId: String, commentId: String)(using auth: AuthToken, ec: ExecutionContext): markAsSpam = new markAsSpam(ws.url(BASE_URL + s"v3/blogs/${blogId}/posts/${postId}/comments/${commentId}/spam").addQueryStringParameters())
	...
	object users {
		...
		object get {
			def apply(userId: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v3/users/${userId}").addQueryStringParameters())
			given Conversion[get, Future[Schema.User]] = (fun: get) => fun.apply()
		}
	}
	object pages {
		class insert(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withPage(body: Schema.Page) = auth.exec(req.withBody(Json.toJson(body)),_.execute("POST")).map(_.json.as[Schema.Page])
		}
		object insert {
			def apply(isDraft: Boolean, blogId: String)(using auth: AuthToken, ec: ExecutionContext): insert = new insert(ws.url(BASE_URL + s"v3/blogs/${blogId}/pages").addQueryStringParameters("isDraft" -> isDraft.toString))
		}
		class publish(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Page]) {
			def apply() = auth.exec(req,_.execute("POST")).map(_.json.as[Schema.Page])
		}
		object publish {
			def apply(blogId: String, pageId: String)(using auth: AuthToken, ec: ExecutionContext): publish = new publish(ws.url(BASE_URL + s"v3/blogs/${blogId}/pages/${pageId}/publish").addQueryStringParameters())
		...
		object delete {
			def apply(useTrash: Boolean, pageId: String, blogId: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(ws.url(BASE_URL + s"v3/blogs/${blogId}/pages/${pageId}").addQueryStringParameters("useTrash" -> useTrash.toString))
			given Conversion[delete, Future[Unit]] = (fun: delete) => fun.apply()
		}
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Page]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.Page])
		}
		object get {
			def apply(blogId: String, view: String, pageId: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v3/blogs/${blogId}/pages/${pageId}").addQueryStringParameters("view" -> view.toString))
			given Conversion[get, Future[Schema.Page]] = (fun: get) => fun.apply()
		}
		class update(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withPage(body: Schema.Page) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PUT")).map(_.json.as[Schema.Page])
		}
		object update {
			def apply(blogId: String, revert: Boolean, pageId: String, publish: Boolean)(using auth: AuthToken, ec: ExecutionContext): update = new update(ws.url(BASE_URL + s"v3/blogs/${blogId}/pages/${pageId}").addQueryStringParameters("revert" -> revert.toString, "publish" -> publish.toString))
		}
		class patch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withPage(body: Schema.Page) = auth.exec(req.withBody(Json.toJson(body)),_.execute("PATCH")).map(_.json.as[Schema.Page])
		}
		object patch {
			def apply(pageId: String, publish: Boolean, blogId: String, revert: Boolean)(using auth: AuthToken, ec: ExecutionContext): patch = new patch(ws.url(BASE_URL + s"v3/blogs/${blogId}/pages/${pageId}").addQueryStringParameters("publish" -> publish.toString, "revert" -> revert.toString))
		}
		class revert(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Page]) {
			def apply() = auth.exec(req,_.execute("POST")).map(_.json.as[Schema.Page])
		}
		object revert {
			def apply(pageId: String, blogId: String)(using auth: AuthToken, ec: ExecutionContext): revert = new revert(ws.url(BASE_URL + s"v3/blogs/${blogId}/pages/${pageId}/revert").addQueryStringParameters())
		...
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.PageList]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.PageList])
		}
		object list {
	...
	object pageViews {
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.Pageviews]) {
			def apply() = auth.exec(req,_.execute("GET")).map(_.json.as[Schema.Pageviews])
		}
		object get {
			def apply(range: String, blogId: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(ws.url(BASE_URL + s"v3/blogs/${blogId}/pageviews").addQueryStringParameters("range" -> range.toString))
