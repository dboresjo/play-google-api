...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object pages {
		class create(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withGoogleFactcheckingFactchecktoolsV1alpha1ClaimReviewMarkupPage(body: Schema.GoogleFactcheckingFactchecktoolsV1alpha1ClaimReviewMarkupPage) = req.withBody(Json.toJson(body)).execute("POST").map(_.json.as[Schema.GoogleFactcheckingFactchecktoolsV1alpha1ClaimReviewMarkupPage])
		}
		object create {
			def apply()(using auth: AuthToken, ec: ExecutionContext): create = new create(auth(ws.url(BASE_URL + s"v1alpha1/pages")).addQueryStringParameters())
		}
		...
		object delete {
			def apply(pagesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): delete = new delete(auth(ws.url(BASE_URL + s"v1alpha1/pages/${pagesId}")).addQueryStringParameters("name" -> name.toString))
		...
		class get(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleFactcheckingFactchecktoolsV1alpha1ClaimReviewMarkupPage]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.GoogleFactcheckingFactchecktoolsV1alpha1ClaimReviewMarkupPage])
		}
		object get {
			def apply(pagesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): get = new get(auth(ws.url(BASE_URL + s"v1alpha1/pages/${pagesId}")).addQueryStringParameters("name" -> name.toString))
		...
		class update(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) {
			def withGoogleFactcheckingFactchecktoolsV1alpha1ClaimReviewMarkupPage(body: Schema.GoogleFactcheckingFactchecktoolsV1alpha1ClaimReviewMarkupPage) = req.withBody(Json.toJson(body)).execute("PUT").map(_.json.as[Schema.GoogleFactcheckingFactchecktoolsV1alpha1ClaimReviewMarkupPage])
		}
		object update {
			def apply(pagesId :PlayApi, name: String)(using auth: AuthToken, ec: ExecutionContext): update = new update(auth(ws.url(BASE_URL + s"v1alpha1/pages/${pagesId}")).addQueryStringParameters("name" -> name.toString))
		}
		class list(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleFactcheckingFactchecktoolsV1alpha1ListClaimReviewMarkupPagesResponse]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.GoogleFactcheckingFactchecktoolsV1alpha1ListClaimReviewMarkupPagesResponse])
		}
		object list {
			def apply(url: String, offset: Int, pageSize: Int, pageToken: String, organization: String)(using auth: AuthToken, ec: ExecutionContext): list = new list(auth(ws.url(BASE_URL + s"v1alpha1/pages")).addQueryStringParameters("url" -> url.toString, "offset" -> offset.toString, "pageSize" -> pageSize.toString, "pageToken" -> pageToken.toString, "organization" -> organization.toString))
	...
	object claims {
		class imageSearch(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.GoogleFactcheckingFactchecktoolsV1alpha1FactCheckedClaimImageSearchResponse]) {
			...
			def apply() = req.execute("GET").map(_.json.as[Schema.GoogleFactcheckingFactchecktoolsV1alpha1FactCheckedClaimImageSearchResponse])
		}
		object imageSearch {
			def apply(imageUri: String)(using auth: AuthToken, ec: ExecutionContext): imageSearch = new imageSearch(auth(ws.url(BASE_URL + s"v1alpha1/claims:imageSearch")).addQueryStringParameters("imageUri" -> imageUri.toString))
