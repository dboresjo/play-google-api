...
class Api @Inject() (ws: WSClient) extends PlayApi {
	...
	object elections {
		...
		class electionQuery(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.ElectionsQueryResponse]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.ElectionsQueryResponse])
		}
		object electionQuery {
			def apply(productionDataOnly: Boolean)(using auth: AuthToken, ec: ExecutionContext): electionQuery = new electionQuery(auth(ws.url(BASE_URL + s"civicinfo/v2/elections")).addQueryStringParameters("productionDataOnly" -> productionDataOnly.toString))
	...
	object divisions {
		class search(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.DivisionSearchResponse]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.DivisionSearchResponse])
		}
		object search {
			def apply(query: String)(using auth: AuthToken, ec: ExecutionContext): search = new search(auth(ws.url(BASE_URL + s"civicinfo/v2/divisions")).addQueryStringParameters("query" -> query.toString))
		...
		class queryDivisionByAddress(private val req: WSRequest)(using auth: AuthToken, ec: ExecutionContext) extends (() => Future[Schema.DivisionByAddressResponse]) {
			def apply() = req.execute("GET").map(_.json.as[Schema.DivisionByAddressResponse])
		}
		object queryDivisionByAddress {
			def apply(address: String)(using auth: AuthToken, ec: ExecutionContext): queryDivisionByAddress = new queryDivisionByAddress(auth(ws.url(BASE_URL + s"civicinfo/v2/divisionsByAddress")).addQueryStringParameters("address" -> address.toString))
